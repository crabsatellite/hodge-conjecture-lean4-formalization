...
/-- Rational equivalence on the trivial example: **honest equality**
on each codim. This is mathematically correct: on a one-point variety
`Spec k`, the Chow group `CH^p(Spec k)` is `ℤ` for `p = 0` (generated
by the fundamental class — no identifications, since there are no
non-constant rational functions whose divisors could relate distinct
multiples of `[Spec k]`) and the zero group for `p ≥ 1` (vanishing
above the dimension). The `Eq` setoid on `ZCyclePUnit p` realises both
cases honestly: equality on `ℤ` for `p = 0` (so `CH^0(Spec k) = ℤ`),
and equality on `PUnit` for `p ≥ 1` (which is trivially `True` because
`PUnit` is a subsingleton — there is only one element to relate).

Replacing the previous `r _ _ := True` (which incorrectly identified
all integers in codim 0) with `Eq` is a structural fix per the
no-trick mandate: instead of using the universal relation as a
catch-all, we use the genuine mathematical relation that happens to
collapse on the codim ≥ 1 case for the right reason (subsingleton). -/
def ratEquivPUnit (p : ℕ) : Setoid (ZCyclePUnit p) :=
...
instance instAlgebraicCycleVarietyData : AlgebraicCycleVarietyData PUnit where
  ...
  ratEquiv_trans := fun h₁ h₂ => h₁.trans h₂
  ratEquiv_symm := fun h => h.symm
