...
- gapPartial `_REQUIRED_HYPOTHESIS` (paper-acknowledged conditional
  ...
  framework atom = Borel-Wallach 2000) + clause (ii.b.2)
  `IsFreudenthalClassPlacedInChernSubalgebra_E7_REQUIRED_HYPOTHESIS`
  (P5-rollback decomposition, R-#new-Phase4: the placement of the
  pulled-back class `[q] ∈ H^8(S_Γ^{tor}, ℚ)` IN the Goresky-Pardon
  Chern subalgebra at degree 8; master tex L11625-11647 explicitly
  records this as not in the published literature; NOT derivable from
  framework + Zucker + Mumford + G-P-EVII alone — well-definedness of
  the subalgebra does not place an arbitrary class in it) +
  ...
  the (iii) ring-hom bridge).
  P5 PATCH HISTORY (R-#new-Phase4): an earlier P5 cascade attempt
  claimed (ii.b) was DERIVED from (ii.a) + Zucker + Mumford framework
  + G-P-EVII, dropping conjectural surface to 3 atoms; Phase 4 audit
  rejected this on three grounds: A3/D1 circularity with clause (iii)
  placement-in-Chern-subalgebra content; A5 paper-Lean inconsistency
  (master tex L11625-11647 explicitly says compatibility "not presently
  available in the published literature"); D2 ceremony retreat (the
  "bridge" was an unproven AXIOM not a theorem; atom-count reduction
  illusory). Rolled back. The honest P5 outcome is: 2 NEW PUBLISHED
  atoms surfaced (Zucker + IH-pullback via BBD 1982 / Saito MHM /
  Goresky-MacPherson 1980) + (ii.b) decomposed into (ii.b.1) PUBLISHED
  IH-pullback + (ii.b.2) `_REQUIRED_HYPOTHESIS` placement; net
  conjectural surface UNCHANGED at 4 typed atoms.
  P7 STRUCTURAL DECOMPOSITION OF G-P-EVII (R-#new): per the P7
  Phase 0+1 hostile-audit FINDING (not a master tex claim), the
  genuine residual content of the monolithic G-P-EVII atom is the
  E_6-representation-theoretic compatibility of G-P §16.2 (the
  K-decomposition step), NOT the abstract §10-12 controlled-form
  framework which Looijenga 2017 Compositio Math. 153 (1349-1371)
  verifies is GROUP-AGNOSTIC (abstract framework via Cor 3.3 + Thm 4.1
  is group-agnostic; Looijenga personally only worked out the
  symplectic case, so the group-agnostic verification is itself
  folkloric-corollary-tier). G-P §1.6 verbatim verified open:
  "We do not know whether the results on Chern classes ... may be
  extended to the 'equal rank' case (when the real rank of G and of
  K coincide)". Literature 2002-2026 confirmed silent on EVII Chern-
  subalgebra extension (Looijenga 2017 only proves abstract framework
  for Sp; Esnault-Harris 2018 excludes EVII as non-abelian-type;
  Burgos-Wildeshaus 2004 gives MHM degeneration not the surjection
  statement; Goresky/Pardon publication-page confirmed no follow-up).
  PAPER-LEAN UNIFICATION DISCLOSURE (A5 audit Pattern check):
  master tex `\\ref{hyp:ChernWeil-bridge-E7}` L11625-11647 records
  only the weaker statement that the degree-8 compatibility is "not
  presently available in the published literature"; the more refined
  identification of §16.2 K-decomposition as the specific bottleneck
  is the P7 PHASE-0 AUDIT's finding (the audit pinpointed the
  bottleneck via direct PDF inspection of G-P §10-12 vs §16.2-16.5
  + Looijenga 2017 Cor 3.3 / Thm 4.1), not a paper-acknowledged
  decomposition. The master tex narrative remains the weaker
  "compatibility-not-published" statement.
  Decomposition (added to OpenHypotheses.lean): 2 NEW PUBLISHED
  sub-atoms `IsBorelHirzebruchClassifyingSpacePresentationFor_E6timesU1_FOLKLORE_PUBLISHED`
  (Borel 1953 + Borel-Hirzebruch 1958 + Mimura-Toda 1991; tier
  `_FOLKLORE_PUBLISHED` per P7 Phase 4 audit Pattern-2 finding
  downgrade) + `IsGPAbstractParabolicConnectionFramework_GroupAgnostic_PUBLISHED`
  (G-P 2002 §10-12 + Looijenga 2017 Cor 3.3 / Thm 4.1) + 1 narrower
  `_REQUIRED_HYPOTHESIS` `IsE6RepresentationCompatibilityOfSection16dot2_REQUIRED_HYPOTHESIS`
  (the genuine load-bearing open piece — see P7-PATCH-A rewire below)
  + 1 INERT downstream-narrowing `IsGPChernSubalgebraSurjectionAtDegree8_E7_REQUIRED_HYPOTHESIS`
  (degree-8 specialization; surfaced as TYPED FUTURE-ATTACK-VECTOR
  PLACEHOLDER per P7 Phase 4 audit Pattern-4 disclosure) + 1 INERT
  alternative `_INVENTION_CLASS` `IsMHMDecompositionTheoremRouteToEVIIChernExtension_OPEN_INVENTION_CLASS`
  (MHM/BBD route; surfaced as TYPED FUTURE-ATTACK-VECTOR PLACEHOLDER).
  Plus 2 bridge axioms (`from_subatoms` and `from_degree8`) routing
  the monolithic atom through the decomposition.
  P7-PATCH-A LOAD-BEARING REWIRE (per P7 Phase 4 audit Pattern-7
  finding = phantom-downstream-user CRITICAL defect, fixed): the
  derived theorem `borel_hirzebruch_schwarz_polynomial_identity_E7_iii_PAPER_LABELLED_CONJECTURAL`
  now discharges the monolithic G-P-EVII input via the bridge
  `goresky_pardon_chern_subalgebra_extension_to_EVII_from_subatoms`
  applied to 2 PUBLISHED sub-atoms + 1 narrower `_REQUIRED_HYPOTHESIS`.
  Net effect: the active conjectural-surface gate for G-P-EVII
  content is NOW the narrower `IsE6RepresentationCompatibilityOfSection16dot2`
  atom (E_6-rep-compat of §16.2 K-decomposition), NOT the monolithic
  `IsGoreskyPardonChernSubalgebraExtensionToEVII` atom (full ring
  extension). The monolithic atom remains in OpenHypotheses.lean as
  a historical predicate but is NO LONGER actively consumed by the
  clause-iii proof path. 4-atom conjectural-surface count UNCHANGED
  but the residual is genuinely narrowed: (i.b.2) INVENTION_CLASS
  + (ii.a) REQUIRED + (ii.b.2) REQUIRED + (P7-active) E_6-rep-compat-
  of-§16.2 REQUIRED instead of (full G-P-EVII extension) REQUIRED.
  P8 FURTHER DECOMPOSITION OF E_6-REP-COMPAT-OF-§16.2 (R-#new-P8):
  Phase 0 hostile audit on the P7-surfaced narrower atom catches
  THREE structural findings: (1) Q3 FALSE — codim-1 boundary of
  EVII is EIII = E_6/Spin(10)·U(1) (itself EXCEPTIONAL E_6 type,
  NOT classical); Wolf 1972 + Satake 1980 + Borel-Ji 2006. This
  INVALIDATES the P7 MHM/BBD INVENTION_CLASS routing-atom premise
  (which assumed all rational boundary components are classical);
  the P7 MHM atom is patched with PREMISE-FALSE annotation.
  (2) Q2 NOT FOUND — form-level Hirzebruch-Mumford proportionality
  for EVII is GENUINELY OPEN (Mumford 1977 = numbers only; Faltings
  1984 + Looijenga 2017 = PEL/Sp form-level only; Gritsenko-Hulek-
  Sankaran 2007 = orthogonal form-level only; EVII non-PEL/non-Sp/
  non-orthogonal/non-abelian-type so NOT covered). This is the
  irreducible gapBlocked-tier content surfaced as typed Lean atom
  `IsHirzebruchMumfordProportionalityFormsForEVII_REQUIRED_HYPOTHESIS`.
  (3) Q1 PARTIAL — V_27 Chern generation of `H^*(BE_6; ℚ)` is
  multi-source folklore (Borel 1953 polynomial-ring framework +
  Toda 1976 mod-p Chern-class identifications + Kono-Mimura
  mid-1970s mod-p assembly); each piece established but the rational
  assembly is folklore-grade. Surfaced as `_FOLKLORE_PUBLISHED`
  typed atoms for both `H^*(BE_6;ℚ)` and `H^*(BE_7;ℚ)`.
  P8 Lean output: 4 new typed atoms
  (`IsEVIIBoundaryStrataClassification_codim1_is_EIII_PUBLISHED` +
  `IsHirzebruchMumfordProportionalityFormsForEVII_REQUIRED_HYPOTHESIS`
  + `IsChernV27GeneratesBE6Rational_FOLKLORE_PUBLISHED` +
  `IsChernV56GeneratesBE7Rational_FOLKLORE_PUBLISHED`) + decomposition
  bridge `is_E6_representation_compatibility_of_section_16dot2_from_subatoms_P8`
  routing the P7-active atom through 1 PUBLISHED + 1 gapBlocked +
  2 FOLKLORE_PUBLISHED. The active conjectural-surface gate after
  P8 is the **form-level HM proportionality for EVII** atom (genuine
...
def gap_hyp_ChernWeil_bridge_E7 : LedgerEntry := {
 ...
 attackHistory := ["R-attacks-#1-R1-clause-i-2026-05-11", "R-attack-#3-R3-clause-ii-2026-05-11", "R-attack-#4-R4-clause-iii-plus-bundled-parent-rollup-2026-05-11", "R-patch-#5.1-hostile-audit-2026-05-11-bundled-signature-refactored-from-clause-iii-only-to-genuine-(i)-AND-(ii)-AND-(iii)-conjunction-paper-faithful", "R-attack-#51-Phase-4-retroactive-audit-MAJOR-DEFECT-OVERCLAIM-vs-paper-paper-labels-clause-i-b-non-vanishing-and-clause-ii-non-cocompact-and-clause-iii-polynomial-identity-all-conjectural-but-Lean-encodes-unconditional-classical-lit-axioms-citing-Borel-Hirzebruch-1958-Matsushima-Borel-Wallach-Mumford-1977-which-do-NOT-directly-establish-specific-claims-status-DOWNGRADED-gapClosed-to-gapPartial-with-paper-labelled-conjectural-disclosure-in-docstrings", "R-attack-#103-clause-i-b-DECOMPOSED-per-Phase-4-recommendation-PUBLISHED-cohomology-atom-IsChernSubringSurjectiveOntoH8_E7P7-plus-INVENTION-CLASS-cross-ring-bridge-atom-IsCrossRingBridgeNonzeroOnFreudenthalQuartic_E7P7_INVENTION_CLASS-plus-bridge-axiom-borel_hirzebruch_..._nonvanish_H8-converted-axiom-to-theorem-tier-of-conjectural-part-refined-PAPER-LABELLED-CONJECTURAL-to-INVENTION-CLASS-clauses-ii-iii-still-monolithic-build-clean-0-sorry-status-stays-gapPartial-Lean-izes-the-R-#102-genuinely-open-finding-per-broken-link-discipline", "R-attack-#104-Phase-0-hostile-lit-review-of-clauses-ii-and-iii-full-theorem-survey-of-Matsushima-Borel-Wallach-Mumford-AMRT-Goresky-Pardon-Faltings-Chai-Lan-Harris-determined-correct-PUBLISHED-vs-REQUIRED-HYPOTHESIS-split-for-each-sub-clause-also-found-clause-iii-REDUCES-TO-i-b-plus-ii-plus-1-PUBLISHED-ring-hom-atom-zero-clause-iii-specific-conjectural-content-and-caught-3-Phase-0-defects-Matsushima-Ann-Math-75-1962-was-phantom-fix-to-Osaka-Math-J-14-Goresky-Pardon-cited-only-as-arXiv-preprint-fix-to-Invent-Math-147-AMRT-PEL-restricted-was-misattribution-of-limitation-flagged-missing-bibitems-GoreskyPardon-Vogan-Zuckerman", "R-attack-#105-REFACTOR-clauses-ii-and-iii-per-R-#104-design-7-new-atom-predicates-plus-7-new-atom-axioms-plus-4-bridge-axioms-old-monolithic-PAPER-LABELLED-CONJECTURAL-axioms-converted-to-derived-theorems-names-kept-build-clean-0-sorry-status-stays-gapPartial-conjectural-surface-cleanly-localised-1-INVENTION-CLASS-i-b-2-plus-2-REQUIRED-HYPOTHESIS-ii-a-extension-and-ii-b-extension-clause-iii-derived-no-own-conjectural-content-R-#51-monolithic-axiom-overclaim-RESOLVED-Phase-0-defects-fixed-in-new-docstrings-formalization-fully-decomposed-and-at-honest-saturation", "R-attack-#106a-Phase-4-structural-audit-found-2-CRITICAL-hidden-identifications-in-iii-bridge-B1-q_G-equals-Phi-q-B2-ii-a-descent-map-equals-iii-ring-hom-plus-2-MILD-folklore-corollary-tier-clarifications-needed", "R-attack-#106b-Phase-4-citation-audit-found-3-MAJOR-G-P-EVII-scope-overclaim-G-P-Baily-Borel-locus-not-toroidal-BW-chapter-mismatch-plus-4-MINOR-Snow-title-Mumford-ring-compat-folklore-Harris-1985-section-2-wrong-bibitem-absence", "R-attack-#107-PATCH-batch-renamed-Mumford-G-P-framework-to-Mumford-only-since-G-P-does-not-cover-EVII-per-G-P-section-1-3-Thm-16-4-and-section-1-6-added-new-REQUIRED_HYPOTHESIS-atom-for-G-P-EVII-extension-consumed-by-iii-bridge-added-new-PUBLISHED-folklore-corollary-atom-for-witness-chain-identifications-iii-bridge-now-7-input-was-5-folklore-corollary-qualifiers-added-Snow-to-Bott-1957-Harris-1985-to-Harris-1989-Proc-LMS-59-G-P-Baily-Borel-locus-disclosed-Mumford-ring-compat-folklore-disclosed-build-clean-0-sorry-status-stays-gapPartial-total-conjectural-surface-now-4-typed-atoms-1-INVENTION_CLASS-i-b-2-plus-3-REQUIRED_HYPOTHESIS-ii-a-ii-b-G-P-EVII-clause-iii-still-derived-no-own-content", "R-attack-#new-P7-Phase-0-1-hostile-audit-on-G-P-EVII-extension-atom-G-P-section-1-6-VERBATIM-VERIFIED-OPEN-We-do-not-know-whether-the-results-on-Chern-classes-may-be-extended-to-the-equal-rank-case-literature-2002-2026-confirmed-silent-Looijenga-2017-Compositio-Math-153-arXiv-1510-04103-proves-abstract-framework-group-agnostic-via-Corollary-3-3-and-Theorem-4-1-but-explicitly-We-work-this-out-in-the-case-of-the-symplectic-group-Esnault-Harris-2018-arXiv-1801-08219-Theorem-2-7-restates-G-P-restriction-extending-only-to-Hodge-type-and-abelian-type-Shimura-EVII-explicitly-excluded-as-non-abelian-type-Burgos-Wildeshaus-2004-Annales-ENS-arXiv-math-0209370-MHM-degeneration-routes-not-the-surjection-statement-Goresky-publication-page-confirms-no-follow-up-Pardon-also-confirms-no-follow-up-DECOMPOSITION-2-NEW-PUBLISHED-sub-atoms-IsBorelHirzebruchClassifyingSpacePresentationFor_E6timesU1_PUBLISHED-Borel-1953-Ann-Math-57-plus-Borel-Hirzebruch-1958-AJM-80-plus-Mimura-Toda-1991-AMS-Transl-91-and-IsGPAbstractParabolicConnectionFramework_GroupAgnostic_PUBLISHED-G-P-2002-section-10-12-plus-Looijenga-2017-Corollary-3-3-Theorem-4-1-plus-1-narrower-REQUIRED_HYPOTHESIS-IsE6RepresentationCompatibilityOfSection16dot2_REQUIRED_HYPOTHESIS-the-genuine-residual-content-K-decomposition-step-of-G-P-section-16-2-for-K-equals-E-6-times-U-1-plus-1-downstream-narrowing-REQUIRED_HYPOTHESIS-IsGPChernSubalgebraSurjectionAtDegree8_E7_REQUIRED_HYPOTHESIS-degree-8-specialization-for-clause-iii-consumer-plus-1-INVENTION_CLASS-IsMHMDecompositionTheoremRouteToEVIIChernExtension_OPEN_INVENTION_CLASS-MHM-BBD-decomposition-route-via-boundary-strata-alternative-attack-vector-plus-2-bridge-axioms-from_subatoms-and-from_degree8-routing-monolithic-atom-through-decomposition-build-clean-0-sorry-status-stays-gapPartial-conjectural-surface-unchanged-at-4-atoms-monolithic-atom-preserved-for-downstream-consumer-stability-but-residual-now-narrower-and-typed-bottleneck-pinpointed-at-G-P-section-16-2-NOT-abstract-section-10-12-framework", "R-attack-#new-P7-Phase-4-hostile-audit-VERDICT-CEREMONY-RETREAT-1-CRITICAL-Pattern-7-phantom-downstream-user-bridges-axioms-from_subatoms-and-from_degree8-never-consumed-by-clause-iii-proof-path-monolithic-G-P-EVII-atom-still-discharged-via-original-witness-and-3-MAJOR-Pattern-2-folkloric-inflation-Borel-Hirzebruch-tier-should-downgrade-to-FOLKLORE-PUBLISHED-Pattern-4-tautological-premise-IsGPChernSubalgebraSurjectionAtDegree8-has-no-content-beyond-monolithic-Pattern-10-A5-paper-Lean-master-tex-L11625-11647-records-only-weaker-not-presently-available-claim-section-16-2-bottleneck-identification-is-P7-audit-finding-not-paper-claim-plus-1-MINOR-Pattern-1-Looijenga-page-range-569-630-wrong-should-be-1349-1371", "R-attack-#new-P7-PATCH-batch-A-load-bearing-rewire-derived-theorem-borel_hirzebruch_schwarz_polynomial_identity_E7_iii_PAPER_LABELLED_CONJECTURAL-now-discharges-monolithic-G-P-EVII-input-via-from_subatoms-bridge-applied-to-2-PUBLISHED-sub-atoms-plus-1-narrower-REQUIRED_HYPOTHESIS-E_6-rep-compat-of-section-16-2-monolithic-atom-no-longer-actively-consumed-active-conjectural-gate-for-G-P-EVII-content-is-NOW-the-narrower-E_6-rep-compat-atom-PATCH-B-tier-downgrade-IsBorelHirzebruchClassifyingSpacePresentationFor_E6timesU1-from-PUBLISHED-to-FOLKLORE_PUBLISHED-PATCH-C-A5-paper-Lean-unification-disclosure-master-tex-records-weaker-not-presently-available-claim-section-16-2-bottleneck-identification-is-P7-audit-finding-not-paper-decomposition-PATCH-D-Looijenga-page-range-corrected-569-630-to-1349-1371-PATCH-E-degree-8-specialization-atom-and-MHM-BBD-routing-INVENTION_CLASS-atom-explicitly-documented-as-INERT-future-attack-vector-typed-placeholders-not-currently-consumed-NET-EFFECT-conjectural-surface-stays-4-atoms-but-residual-genuinely-narrowed-from-monolithic-G-P-EVII-extension-to-E_6-rep-compat-of-section-16-2-K-decomposition-step-narrower-content-build-clean-0-sorry-status-stays-gapPartial-ceremony-retreat-charge-disarmed-via-PATCH-A-load-bearing-rewire", "R-attack-#new-P8-Phase-0-hostile-audit-on-P7-surfaced-narrower-atom-IsE6RepresentationCompatibilityOfSection16dot2_REQUIRED_HYPOTHESIS-THREE-structural-findings-Q3-FALSE-codim-1-boundary-of-EVII-is-EIII-equals-E_6-mod-Spin-10-times-U-1-itself-EXCEPTIONAL-E_6-type-NOT-classical-Wolf-1972-plus-Satake-1980-plus-Borel-Ji-2006-this-INVALIDATES-the-P7-MHM-BBD-INVENTION_CLASS-routing-atom-premise-which-assumed-all-rational-boundary-components-are-classical-P7-MHM-atom-patched-with-PREMISE-FALSE-annotation-Q2-NOT-FOUND-form-level-Hirzebruch-Mumford-proportionality-for-EVII-is-GENUINELY-OPEN-Mumford-1977-equals-numbers-only-Faltings-1984-plus-Looijenga-2017-equals-PEL-Sp-form-level-only-Gritsenko-Hulek-Sankaran-2007-equals-orthogonal-form-level-only-EVII-non-PEL-non-Sp-non-orthogonal-non-abelian-type-NOT-covered-irreducible-gapBlocked-tier-content-surfaced-as-typed-Lean-atom-IsHirzebruchMumfordProportionalityFormsForEVII_REQUIRED_HYPOTHESIS-Q1-PARTIAL-V_27-Chern-generation-of-H-BE_6-Q-is-multi-source-folklore-Borel-1953-polynomial-ring-framework-plus-Toda-1976-mod-p-Chern-class-identifications-plus-Kono-Mimura-mid-1970s-mod-p-assembly-each-piece-established-but-rational-assembly-is-folklore-grade-surfaced-as-FOLKLORE_PUBLISHED-typed-atoms-for-both-H-BE_6-Q-and-H-BE_7-Q-Lean-output-4-new-typed-atoms-IsEVIIBoundaryStrataClassification_codim1_is_EIII_PUBLISHED-plus-IsHirzebruchMumfordProportionalityFormsForEVII_REQUIRED_HYPOTHESIS-plus-IsChernV27GeneratesBE6Rational_FOLKLORE_PUBLISHED-plus-IsChernV56GeneratesBE7Rational_FOLKLORE_PUBLISHED-plus-decomposition-bridge-is_E6_representation_compatibility_of_section_16dot2_from_subatoms_P8-routing-P7-active-atom-through-1-PUBLISHED-plus-1-gapBlocked-plus-2-FOLKLORE_PUBLISHED-plus-P7-MHM-BBD-routing-atom-patched-with-PREMISE-FALSE-annotation-build-clean-0-sorry-conjectural-surface-stays-4-atoms-but-residual-now-pinpointed-at-form-level-HM-proportionality-for-EVII-as-the-structural-barrier-this-is-a-genuine-OPEN-published-literature-silent-problem-typed-out-per-failure-as-asset-discipline"]
 obstacleCitation := some "POST-R-#new-P7-PATCH (post-Phase-4-audit ceremony-retreat charge fully disarmed via PATCH-A load-bearing rewire): clauses (i.b), (ii.a), (ii.b), (iii) all fully decomposed; R-#106a/b + P5-rollback + P7 audit patches A-E applied. Conjectural surface = 4 typed atoms (count UNCHANGED; CONTENT GENUINELY NARROWED): (i.b.2) `IsCrossRingBridgeNonzeroOnFreudenthalQuartic_E7P7_INVENTION_CLASS` (literature-absent cross-ring map; canonical geometric Φ gives 0, Landsberg-Manivel 2001); (ii.a) `IsFreudenthalClassRealizedByGInvariantCohomology_E7_REQUIRED_HYPOTHESIS` (specific-`[q]_G` realisation at degree 8); (ii.b.2) `IsFreudenthalClassPlacedInChernSubalgebra_E7_REQUIRED_HYPOTHESIS` (P5-rollback: placement-in-Chern-subalgebra residual at degree 8); (NEW P7-ACTIVE) `IsE6RepresentationCompatibilityOfSection16dot2_REQUIRED_HYPOTHESIS` (the genuine residual content of the G-P-EVII extension after the P7 decomposition: the E_6-representation-theoretic compatibility of G-P §16.2 K-decomposition step for `K = E_6 × U(1)` with `E_6` as a single irreducible factor; NARROWER than the full G-P-EVII ring extension; load-bearing in the active clause-iii proof path via P7-PATCH-A rewire). The monolithic `IsGoreskyPardonChernSubalgebraExtensionToEVII_REQUIRED_HYPOTHESIS` remains in OpenHypotheses.lean as a historical predicate but is NO LONGER actively consumed; its content is now routed through the 2 PUBLISHED sub-atoms + new E_6-rep-compat atom via the load-bearing bridge `goresky_pardon_chern_subalgebra_extension_to_EVII_from_subatoms`. INERT future-attack-vector typed placeholders surfaced for completeness: `IsGPChernSubalgebraSurjectionAtDegree8_E7_REQUIRED_HYPOTHESIS` (degree-8 specialization) + `IsMHMDecompositionTheoremRouteToEVIIChernExtension_OPEN_INVENTION_CLASS` (MHM/BBD route via boundary strata). Clause (iii) has no own obstacle (REDUCES-TO via the 7-input bridge incorporating the 4 conjectural atoms above + 3 PUBLISHED folklore-corollary atoms). All 4 old monolithic `_PAPER_LABELLED_CONJECTURAL` axioms now derived theorems (names kept). PUBLISHED atoms (POST-P5-rollback + P7-PATCH): Schwarz 1978; Borel 1953 + Borel-Hirzebruch 1958 + Bott 1957; Mimura-Toda 1991 (Lie group cohomology incl. E_6, via `_FOLKLORE_PUBLISHED` per P7-PATCH-B tier-downgrade); Borel-Wallach 2000 (folklore-corollary); Mumford 1977 + AMRT 2010; Zucker (Looijenga 1988 / Saper-Stern 1990 type-uniform); IH-pullback (BBD 1982 + Saito MHM 1988 + Goresky-MacPherson 1980); G-P §10-12 abstract framework (group-agnostic per Looijenga 2017 Compositio Math. 153 (1349-1371) Cor 3.3 / Thm 4.1); ring-hom-compatibility folklore-corollary; witness-chain-identification folklore-corollary."
}
...
def gap_ChernWeil_bridge_E7_i : LedgerEntry := {
 ...
 attackHistory := ["R-attack-#1-Phase-1-2026-05-11-1A-Schwarz-VERIFIED-1B-BorelHirzebruch-VERIFIED-1C-triple-source-VERIFIED-Phase-2-Lean-writer-closed-no-sorry-2026-05-11", "R-attack-#51-Phase-4-retroactive-audit-MAJOR-DEFECT-clause-i-b-paper-labels-non-vanishing-conjectural-but-Lean-axiom-typed-unconditional-status-DOWNGRADED-gapClosed-to-gapPartial", "R-attack-#101-global-re-survey-Plan-agent-identified-clause-i-b-as-the-ONLY-remaining-headroom-gap-in-the-formalization-hunch-it-might-be-a-finite-Schubert-calculus-computation-rather-than-a-genuine-open-conjecture", "R-attack-#102-Constructor-Phase-0-no-published-source-establishes-i-b-checked-Schwarz-Borel-Borel-Hirzebruch-Atiyah-Bott-BGG-Brion-Iliev-Manivel-Cayley-plane-Bott-1965-Kobayashi-Nomizu-Landsberg-Manivel-no-phantom-citations-Phase-1-the-cohomology-side-is-fully-RIGID-dim-H-8-E7-P7-Q-equals-1-equals-Q-h-4-triple-confirmed-via-Poincare-quotient-and-minuscule-weight-poset-V_56-down-E6-equals-1-27-27bar-1-charge-3-to-1-Chern-roots-3h-h-27-minus-h-27-minus-3h-c_2-equals-minus-36-h-2-c_4-equals-594-h-4-Chern-subring-surjects-onto-H-8-BUT-the-cross-ring-bridge-Phi-Sym-4-V_56-star-E7-to-H-8-is-literature-absent-and-the-canonical-geometric-Phi-restrict-q-to-O-minus-1-gives-Phi-q-equals-q-v-omega-7-times-h-4-equals-0-because-q-vanishes-on-the-closed-orbit-E7-P7-subset-q-equals-0-tangential-variety-Landsberg-Manivel-J-Algebra-239-2001-Prop-5-8-VERDICT-GENUINELY-OPEN-paper-PAPER-LABELLED-CONJECTURAL-tag-CORRECT-not-too-cautious-no-Lean-retitle-warranted-formalization-at-honest-saturation-on-this-gap", "R-attack-#103-fresh-hostile-audit-CONFIRMED-R-#102-no-closing-citation-checked-Vinberg-theta-groups-all-Iliev-Manivel-and-Landsberg-Manivel-papers-incl-magic-square-and-Cayley-plane-Chaput-Manivel-Perrin-Sato-Kimura-Garibaldi-Goresky-Pardon-Friedman-Laza-Gross-Landsberg-Manivel-Prop-5-8-actively-confirms-q-vanishes-on-E7-P7-the-negative-direction-rigid-computations-re-verified-exactly-2-genuine-findings-both-honest-DOWNGRADES-1-reclassify-clause-i-b-conjectural-part-one-tier-down-PAPER-LABELLED-CONJECTURAL-to-INVENTION-CLASS-same-situation-as-SG-22-and-hecke_bbt_c-2-the-Phi-refactor-is-warranted-split-IsBorelHirzebruchNonvanishH8-into-PUBLISHED-Chern-subring-atom-plus-INVENTION-CLASS-cross-ring-bridge-atom-REFACTOR-DONE-this-round-IsBorelHirzebruchNonvanishH8-now-DERIVED-via-bridge-axiom-borel_hirzebruch_1958_freudenthal_nonvanish_H8-converted-axiom-to-theorem-build-clean-0-sorry-status-stays-gapPartial-no-promotion-this-Lean-izes-the-genuinely-open-finding-per-broken-link-discipline", "R-attack-#P2-frontal-attack-on-i-b-2-INVENTION-CLASS-tried-3-non-canonical-Phi-readings-Atiyah-Bott-equivariant-localization-at-56-T-fixed-points-Sato-Kimura-PVS-b-function-via-Kashiwara-Saito-Vinberg-theta-group-equivariant-cohomology-cocycle-all-3-give-0-or-wrong-degree-or-collapse-to-Chern-subring-PLUS-new-structural-theorem-WEIGHT-GRADING-VANISHING-for-minuscule-V-and-G-invariant-p-of-degree-d-p-v_w-equals-0-for-all-weight-vectors-strengthens-LM-2001-Prop-5-8-in-proof-method-and-scope-PUBLISHED-folklore-corollary-Bourbaki-Lie-VIII-plus-textbook-T-grading-applied-E_7-V_56-q-4-gives-q-vanishes-at-all-56-T-fixed-points-PLUS-salvage-path-identified-reinterpret-q_G-as-c_4-V_56-equals-594-h-4-or-c_2-V_56-squared-equals-1296-h-4-already-in-Chern-subring-i-b-1-PUBLISHED-would-trivially-close-i-b-2-but-dissolves-INVENTION-CLASS-content-into-definitional-fiat", "R-attack-#P2-Phase-4-audit-VERDICT-CONFIRMED-Weight-Grading-Vanishing-theorem-rigorous-proof-clean-V-no-zero-weight-plus-T-equivariance-plus-homogeneity-implies-p-v_w-equals-0-A1-A4-all-checked-PUBLISHED-folklore-corollary-tier-acceptable-Bourbaki-Lie-VIII-section-7-3-plus-Humphreys-Linear-Algebraic-Groups-section-13-T-equivariance-of-invariants-strengthens-LM-2001-Prop-5-8-in-proof-simplicity-and-scope-not-conclusion-since-LM-already-gives-full-variety-which-includes-56-fixed-points-B1-Atiyah-Bott-direct-corollary-of-Weight-Grading-Vanishing-B2-b-function-reading-OK-but-citation-correction-needed-Ukai-2003-Compositio-135-does-NOT-exclude-E_7-Ukai-covers-all-exceptional-types-for-Dynkin-Kostant-PVS-but-E_7-V_56-Freudenthal-quartic-is-NOT-a-Dynkin-Kostant-PVS-it-is-scalar-extension-PVS-classified-in-Sato-Kimura-1977-so-Ukai-doesn-t-compute-it-conclusion-wrong-degree-still-stands-B3-Vinberg-theta-OK-Z2-grading-e_8-equals-e_7-plus-sl_2-plus-V_56-tensor-V_2-theta-rep-is-V_56-tensor-V_2-112-dim-NOT-V_56-Casimir-squared-is-already-c_2-V_56-squared-in-Chern-subring-D1-no-2024-2026-paper-found-with-new-Phi-construction-CITATION-FIX-corrected-Ukai-attribution-in-docstring-SALVAGE-PATH-Phase-4-recommends-Option-gamma-PARTIAL-INFORMATIONAL-ONLY-keep-INVENTION-CLASS-tier-do-not-promote-i-b-2-to-closure-record-salvage-in-docstring-defer-master-tex-rewording-to-future-coordination-round", "R-attack-#P2-PATCH-Lean-ized-Weight-Grading-Vanishing-Theorem-added-IsWeightGradingVanishingTheorem_minuscule-PUBLISHED-folklore-corollary-atom-plus-witness-axiom-weight_grading_vanishing_theorem_minuscule_PUBLISHED-cited-Bourbaki-Lie-VIII-section-7-3-plus-Humphreys-textbook-T-grading-applied-also-to-SG-5-V_56-odd-Hodge-weight-parity-via-IsV56OddWeightForcesHevenAbsence_sg5-from-R-#new-i-b-2-docstring-upgraded-to-record-full-5-reading-non-existence-with-corrected-Ukai-attribution-plus-salvage-path-Option-gamma-disclosure-build-clean-0-sorry-status-i-b-2-stays-gapPartial-INVENTION-CLASS-no-promotion-formalization-honest-saturation-on-this-gap-confirmed"]
...
def gap_ChernWeil_bridge_E7_ii : LedgerEntry := {
 ...
 computability := "POST-R-#new-Phase4 (P5 ROLLBACK + correct decomposition): clause (ii.a) PUBLISHED framework (Borel-Wallach 2000) + `_REQUIRED_HYPOTHESIS` for specific-`[q]_G` realisation at degree 8 (UNCHANGED). Clause (ii.b) now DECOMPOSED into (ii.b.1) PUBLISHED `IsIHPullbackToToroidalForFreudenthalClass_E7_PUBLISHED` (BBD 1982 + Saito MHM 1988 + Goresky-MacPherson 1980; corrects R-#108 Phantom Attribution to 'Looijenga §3') + (ii.b.2) `IsFreudenthalClassPlacedInChernSubalgebra_E7_REQUIRED_HYPOTHESIS` (the GENUINE residual: placement of pulled-back class IN Chern subalgebra; master tex L11625-11647 explicitly says 'not presently available in the published literature'). The (ii.b) compatibility witness is a derived theorem via the (ii.b.1)+(ii.b.2) structural bridge. Plus auxiliary PUBLISHED atom Zucker (Looijenga 1988 / Saper-Stern 1990, type-uniform for EVII). Net conjectural surface UNCHANGED at 4 typed atoms (1 _INVENTION_CLASS + 3 _REQUIRED_HYPOTHESIS). The earlier R-#108 cascade-to-3-atoms claim was rejected by Phase 4 audit (circularity with (iii) placement-claim + ceremony retreat + paper-Lean inconsistency). Honest outcome: 2 NEW PUBLISHED atoms surfaced + (ii.b) decomposed; no closure."
 attackVector := "POST-R-#new-Phase4: status gapPartial. Local clause-(ii) conjectural content = 2 `_REQUIRED_HYPOTHESIS` atoms — (ii.a) specific-`[q]_G` realisation at degree 8; (ii.b.2) placement of pulled-back class in Chern subalgebra at degree 8. The (ii.b.1) IH-pullback step is now PUBLISHED (BBD/Saito/G-MP). Plus auxiliary PUBLISHED Zucker (Looijenga/Saper-Stern). The P5 ROLLBACK preserves the structural surfacing (more PUBLISHED machinery exposed) while honestly retaining the genuine conjectural residue. The placement-in-Chern-subalgebra step is the irreducible content that master tex L11625-11647 explicitly flags as not-published; until either a published placement argument is found OR the salvage path (define `[q]_G := c_4(𝓥_56^can)`) is formally adopted via master-tex coordination, (ii.b.2) stays `_REQUIRED_HYPOTHESIS`."
 attackHistory := ["R-attack-#3-Phase-1-2026-05-11-Matsushima-Mumford-triple-source-verified-Phase-2-Lean-writer-closed-no-sorry-conjunction-intro-2026-05-11", "R-attack-#51-Phase-4-retroactive-audit-MAJOR-DEFECT-clause-ii-non-cocompact-boundary-paper-labels-conditional-but-Lean-axioms-typed-unconditional-status-DOWNGRADED-gapClosed-to-gapPartial", "R-attack-#104-Phase-0-hostile-lit-review-full-theorem-survey-of-Matsushima-1962-1967-Borel-Wallach-1980-Mumford-1977-AMRT-1975-2010-Goresky-Pardon-2002-Faltings-Chai-Lan-Harris-1985-determined-the-correct-PUBLISHED-vs-REQUIRED-HYPOTHESIS-split-for-each-sub-clause-also-caught-3-Phase-0-defects-Matsushima-Ann-Math-75-1962-312-330-was-a-phantom-fix-to-Osaka-Math-J-14-1962-1-20-Goresky-Pardon-cited-only-as-arXiv-preprint-fix-to-Invent-Math-147-2002-561-612-AMRT-PEL-restricted-was-misattribution-of-limitation-PEL-belongs-to-Faltings-Chai-Lan-not-AMRT-also-flagged-missing-bibitems-GoreskyPardon-Vogan-Zuckerman-in-master-tex", "R-attack-#105-REFACTOR-clause-ii-decomposed-per-R-#104-design-4-new-atom-predicates-IsBorelWallachStableInvariantDescentFramework_E7-PUBLISHED-IsFreudenthalClassRealizedByGInvariantCohomology_E7_REQUIRED_HYPOTHESIS-IsMumfordCanonicalExtensionFramework_E7-PUBLISHED-IsFreudenthalClassExtendsCompatiblyAtDegree8_E7_REQUIRED_HYPOTHESIS-plus-4-atom-axioms-plus-2-bridge-axioms-old-PAPER-LABELLED-CONJECTURAL-axioms-converted-to-derived-theorems-names-kept-build-clean-0-sorry-status-stays-gapPartial-Phase-0-defects-fixed-in-new-docstrings-Matsushima-citation-AMRT-PEL-misattribution-Goresky-Pardon-Invent-Math-147", "R-attack-#106a-Phase-4-structural-audit-on-R-#105-found-2-CRITICAL-gaps-B1-B2-iii-bridge-silently-bundled-q_G-equals-Phi-q-and-ii-a-descent-map-equals-iii-ring-hom-identifications-no-Lean-atom-carried-them-plus-2-MILD-folklore-corollary-tier-clarifications-on-BW-framework-and-ring-hom-glue-atoms", "R-attack-#106b-Phase-4-citation-audit-on-R-#105-found-3-MAJOR-Goresky-Pardon-2002-scope-restricted-to-Sp_n-U-p-q-SO-2n-SO-2-p-NOT-EVII-per-G-P-section-1-3-Thm-16-4-and-G-P-section-1-6-explicitly-open-equal-rank-extension-G-P-classes-on-Baily-Borel-not-toroidal-BW-chapter-VII-vs-XIII-XIV-mismatch-plus-4-MINOR-Snow-1986-title-Mumford-1977-ring-compat-folklore-Harris-1985-section-2-wrong-Snow-Landsberg-Manivel-bibitem-absence", "R-attack-#107-PATCH-batch-of-R-#106a-and-R-#106b-findings-renamed-IsMumfordGoreskyPardonCanonicalExtensionFramework_E7-to-IsMumfordCanonicalExtensionFramework_E7-since-G-P-does-not-cover-EVII-added-new-REQUIRED_HYPOTHESIS-atom-IsGoreskyPardonChernSubalgebraExtensionToEVII_REQUIRED_HYPOTHESIS-surfacing-G-P-section-1-6-open-equal-rank-extension-consumed-by-iii-bridge-not-by-ii-b-added-new-PUBLISHED-folklore-corollary-atom-IsCompactDualQuarticImageAndDescentMapWitnessChain_E7_FOLKLORE_PUBLISHED-handling-B1-B2-identifications-iii-bridge-now-7-input-was-5-folklore-corollary-qualifier-added-to-BW-framework-and-ring-hom-glue-atom-docstrings-Snow-1986-replaced-by-Bott-1957-Ann-Math-66-Harris-1985-section-2-replaced-by-Harris-1989-Proc-LMS-59-G-P-Baily-Borel-locus-disclosed-Mumford-1977-ring-compat-folkloric-upgrade-disclosed-build-clean-0-sorry-status-stays-gapPartial-conjectural-surface-now-4-typed-atoms-1-INVENTION_CLASS-plus-3-REQUIRED_HYPOTHESIS", "R-attack-#108-P5-Constructor-claimed-cascade-clause-ii-b-DERIVED-from-ii-a-plus-Zucker-plus-Mumford-plus-G-P-EVII-conjectural-surface-drops-from-4-to-3-atoms-added-IsZuckerConjectureL2EqualsIH-PUBLISHED-plus-bridge-axiom-freudenthal-class-extends-compatibly-at-degree8-E7-FROM-iia-and-GP-EVII-plus-converted-axiom-to-derived-theorem", "R-attack-#new-Phase4-audit-on-R-#108-REJECTED-cascade-3-CRITICAL-failures-A3-D1-CIRCULARITY-the-bridge-axiom-implicitly-asserts-placement-of-q-in-Chern-subalgebra-which-is-the-polynomial-identity-content-of-clause-iii-and-iii-consumes-ii-b-A5-PAPER-LEAN-INCONSISTENCY-master-tex-L11625-11647-explicitly-says-degree-8-boundary-compatibility-not-presently-available-in-the-published-literature-directly-contradicting-cascade-derivability-claim-D2-CEREMONY-RETREAT-bridge-freudenthal-class-extends-compatibly-at-degree8-E7-FROM-iia-and-GP-EVII-is-an-unproven-AXIOM-not-a-theorem-atom-count-reduction-illusory-net-conjectural-surface-unchanged-this-is-exactly-feedback-team-v4-ceremony-is-retreat-failure-mode-plus-A2-Phantom-Attribution-Looijenga-1988-section-3-cited-for-IH-pullback-correct-anchor-is-BBD-1982-Saito-MHM-1988-Goresky-MacPherson-1980", "R-attack-#new-Phase4-PATCH-ROLLBACK-cascade-axiom-freudenthal_class_extends_compatibly_at_degree8_E7_FROM_iia_and_GP_EVII-removed-replaced-with-honest-decomposition-of-ii-b-into-ii-b-1-PUBLISHED-IsIHPullbackToToroidalForFreudenthalClass_E7_PUBLISHED-anchors-BBD-1982-plus-Saito-MHM-1988-plus-Goresky-MacPherson-1980-correct-citation-and-ii-b-2-REQUIRED_HYPOTHESIS-IsFreudenthalClassPlacedInChernSubalgebra_E7_REQUIRED_HYPOTHESIS-residual-conjectural-content-placement-in-Chern-subalgebra-paper-acknowledged-not-in-published-literature-plus-purely-structural-definitional-bridge-axiom-freudenthal_class_extends_compatibly_at_degree8_E7_FROM_iib1_and_iib2-derived-theorem-preserved-with-honest-input-pair-Zucker-PUBLISHED-atom-from-R-#108-preserved-as-auxiliary-genuine-published-surfacing-conjectural-surface-stays-4-typed-atoms-1-INVENTION_CLASS-i-b-2-plus-3-REQUIRED_HYPOTHESIS-ii-a-ii-b-2-G-P-EVII-honest-outcome-2-NEW-PUBLISHED-atoms-Zucker-plus-IH-pullback-and-ii-b-decomposed-but-no-closure"]
 obstacleCitation := some "POST-R-#new-Phase4 (P5 rollback): clause (ii) honestly decomposed. PUBLISHED atoms: `IsBorelWallachStableInvariantDescentFramework_E7` (Borel-Wallach 2000) + `IsMumfordCanonicalExtensionFramework_E7` (Mumford 1977 + AMRT 2010, G-P 2002 explicitly excluded for EVII per G-P §1.6) + `IsZuckerConjectureL2EqualsIH_PUBLISHED` (Looijenga 1988 / Saper-Stern 1990 type-uniform) + `IsIHPullbackToToroidalForFreudenthalClass_E7_PUBLISHED` (BBD 1982 + Saito MHM 1988 + Goresky-MacPherson 1980; corrects R-#108 Phantom Attribution). Open conjectural content (2 LOCAL `_REQUIRED_HYPOTHESIS` atoms): (a) `IsFreudenthalClassRealizedByGInvariantCohomology_E7_REQUIRED_HYPOTHESIS` — (ii.a); (b) `IsFreudenthalClassPlacedInChernSubalgebra_E7_REQUIRED_HYPOTHESIS` — (ii.b.2), the residual placement-in-Chern-subalgebra content. Both paper-acknowledged conditional (master tex L11605-11647 + L11625-11647). Plus globally-counted `IsGoreskyPardonChernSubalgebraExtensionToEVII_REQUIRED_HYPOTHESIS` consumed by (iii) ring-hom bridge."
}
...
def gap_SG_18 : LedgerEntry := {
 ...
 attackHistory := ["R-attack-#26-Phase-0-hostile-lit-audit-full-theorem-survey-Murre-1990-conjecture-parts-A-B-C-D-correctly-identified-B-i-3-component-as-operative-not-full-B-also-caught-master-tex-L9419-9420-paper-level-defect-Murre-B-equivalent-to-algebraicity-of-pi-3-strictly-false-since-Murre-B-asserts-all-pi-i-simultaneously-only-i-3-component-operative-Phase-2-Lean-writer-3-framework-axioms-Deligne-1980-HL-rational-cohomological-pi-3-Kunneth-master-tex-prop-omega-diagonal-eq-omega-from-delta-plus-1-NAMED-OPEN-extension-axiom-Murre-B-i-3-component-typed-bridge-master-tex-integration-lem-sg18-pi3-chow-conditional-12-lines-after-prop-omega-diagonal-outer-proof-Phase-4-MEDIUM-from-R25-honored-lemma-placed-outside-outer-proof-also-added-6-missing-bibitems-Murre1990-MurreNagelPeters2013-Shermenev1974-DeningerMurre1991-Kunnemann1994-Deligne80-Jannsen-existing-bibitem-reused", "R-attack-#P4-frontal-attack-on-SG-18-Murre-B-i-3-via-3-readings-A-Mackey-isotypic-V_56-projection-of-Delta_X-B-Kuga-Sato-motive-AGHMP-style-E_7-modularity-C-Vinberg-theta-group-E_7-V_56-tensor-V_2-genus-2-hyperelliptic-VERDICT-OPEN-INERT-Pattern-vi-self-reduction-Reading-A-CIRCULAR-the-V_56-Mackey-isotypic-component-of-Delta_X-projects-onto-the-Freudenthal-symplectic-line-omega-which-is-the-Main-Theorem-target-itself-master-tex-L9911-9912-EXPLICITLY-records-the-equivalence-i-3-component-of-Murre-B-for-X-equivalent-to-algebraicity-of-omega-at-Chow-level-Reading-B-literature-absent-Milne-2011-arXiv-1105-0887-section-10-EXPLICITLY-excludes-E_6-E_7-from-abelian-motive-type-Shimura-AGHMP-2017-Compositio-153-3-and-Howard-Madapusi-2022-arXiv-2211-05108-cover-orthogonal-signature-n-2-only-Madapusi-2024-26-preprints-arXiv-2405-12392-foundational-only-no-Chow-Kunneth-content-Greer-Tayou-2026-arXiv-2603-01251-survey-keeps-exceptional-Shimura-modularity-conjectural-Reading-C-scope-mismatch-Bhargava-Gross-2014-Thorne-2013-Romano-Thorne-2018-Vinberg-theta-group-parametrises-genus-2-hyperelliptic-Jacobian-moduli-dim-2-abelian-NOT-the-rigid-5-dim-EVII-phantom-Net-cascade-0-promotions-0-downgrades-1-epistemic-sharpening-circularity-now-ledger-documented-alongside-paper-self-record", "R-attack-#P4-PATCH-Lean-ized-Pattern-vi-self-reduction-finding-added-atom-IsV56MackeyDeltaXEquivToOmegaAlgebraicity_sg18_NAMED_OPEN_CIRCULAR-plus-witness-axiom-sg18_p4_pattern_vi_self_reduction_obstruction-tier-NAMED_OPEN_CIRCULAR-records-the-structural-obstruction-without-promoting-SG-18-status-stays-gapPartial-NAMED-OPEN-paper-source-citation-master-tex-L9911-9912-verified-Murre-B-i-3-component-equivalent-to-algebraicity-of-omega-at-Chow-level-build-clean-0-sorry"]
 obstacleCitation := some "POST-R-attack-#P4 (2026-05-13): Chow-level lift of cohomological π_3 ∈ CH^d(X × X)_ℚ is NAMED-OPEN-CIRCULAR. Murre 1990 J. reine angew. Math. 409 (i=3 component of conj B; not closed for rigid exceptional-MT X with dim ≥ 5; closure mechanisms known for abelian-type X via Shermenev 1974 / Deninger-Murre 1991 / Künnemann 1994). P4 attack confirmed three structural barriers: (A) Pattern (vi) self-reduction — the V_56-Mackey-isotypic projection of Δ_X onto the trivial-summand `1 ⊂ V_56 ⊗ V_56` (= symplectic line ω) IS the Main Theorem target itself per master tex L9911-9912's own equivalence; (B) Milne 2011 §10 EXPLICITLY excludes E_6/E_7 from abelian-motive-type Shimura (AGHMP/Howard-Madapusi orthogonal Chow-modularity has no published E_7 extension in 1990-2026); (C) Vinberg (E_7, V_56 ⊗ V_2) θ-group parametrises genus-2 hyperelliptic Jacobian moduli (dim-2 abelian), scope mismatch with rigid 5-dim EVII phantom. Net cascade: 0 promotions, 0 downgrades, 1 epistemic sharpening — the circularity is now ledger-documented."
}
