...
* P92: `chern_pairing_deg4_constraint` --Lean-verified via norm_num on the
  P48 explicit Chern-class Zcoefficients.
...
Hyp_BorelMAtLeast8 the j^8 iso makes H^8(S_Γ; Z_G 1-dim coming from the
TRIVIAL g-module h^4 Kähler class (Cartan theorem H^*(g,K;Z =
H^*(Ě_VII;Z; b_8(Ě_VII) = 1 from Borel-Hirzebruch Poincaré poly). Any
...
"R(q)=8 NEVER ACHIEVED" verdict was based on dim_C(u forAllk_C) enumeration,
which is NOT V-Z R(q) := dim_C(u forAllp_C); reframed under P36 audit.
...
K_forAllisotypic V_56 = L_{+3} ?E_{+1} ?E_{-1} ?L_{-3}.
...
extends to c_1(L̄)^4 forAllChern subring of H^*(S_Γ^tor) via Mumford canonical
...
Sym^4(t^forAll^{W(E_7)}_+, the augmentation ideal that the Borel-Hirzebruch
coinvariant presentation H^*(Ě_VII) = Sym(t^forAll^{W(E_6)}/(Sym(t^forAll^{W(E_7)}_+)
...
Sym^m N^forAll (N = normal bundle). This Φ_jet(q) is the genuine bridge --
...
(1-h)·c(𝓔_{+1})·c(𝓔_{-1})·(1+h) = 1; and by the Hodge pairing 27' = 27^forAll
so 𝓔_{-1} ~𝓔_{+1}^forAll giving c(𝓔_{+1})·c(𝓔_{+1}^forAll = 1/(1-h^2). Degree
by degree: 2c_2(𝓔_{+1}) - c_1(𝓔_{+1})^2 = h^2, and the degree-4 analogue.
Since c_i(V_56^{can}) = 0, the master tex's [q] = P(c_1,...,c_4) means
P(c_i(𝓔_{+1})), and [q]_G = P(c_i(𝓔_{+1})) forAllH^8(Ě_VII) = ℚ·h^4.
...
evaluates q on the Hodge-graded Chern roots {-h} forAll{x_i} forAll{-x_i} forAll{+h}
...
Eisenstein layer-codim synthesis. For arithmetic Γ ?E_{7(-25)}(Z, the
...
proper Zparabolics P, and each layer is supported at total degrees
...
content). E_7 root-system structural fact: every proper Zparabolic of
...
layer contributes zero, giving H^8_Eis(S_Γ; Z = 0. (Q-rank-0 case is
...
"1-dim H^8(S_Γ; Z_G" reading (which would have required surjectivity =
...
48 c_2² in H^8(S_Γ^{tor}; Z, where the c_i are Chern classes of
...
cartan_1929_compact_dual_iso (= H^*(g, K; Z = H^*(Ě_VII; Z; supplied
 ...
 **DEAD-END (P56 BYPASSED, 2026-05-15; P95 RE-AFFIRMED 2026-05-16, no
 longer load-bearing)**: this opaque Zcarrier is referenced ONLY by the
...
/-- **Cat 1 derivation-stage (§3.4.2, P232 LEAN-CLOSED)** --H^8 of compact
 ...
 `H^8(Ě_VII; Z` entirely in the `(4,4)`-piece.
...
/-- **Cat 1 derivation-stage (§3.4.2, P231 LEAN-CLOSED)** --Borel 1974
 ...
 homomorphism `j^q : H^q(Ě_VII; Z ->H^q(S_Γ; Z^G` is INJECTIVE up
...
def cohomologyIso_at_deg8 : Prop :=
  ∀ (A : Type) [AddCommGroup A] [Module ZA]
    (B : Type) [AddCommGroup B] [Module ZB]
    [Infrastructure.Cohomology.MatsushimaData A B],
    Function.Injective
        (Infrastructure.Cohomology.MatsushimaData.j_q (A := A) (B := B))
      forAll8 ~Infrastructure.Cohomology.MatsushimaData.injective_range
              (A := A) (B := B)
 ...
 **LEAN-CLOSED**: previously an `opaque Prop` hypothesis predicate. Now
 ...
 a designated G-invariants submodule `G_invariants : Submodule ZA`, and
...
def freudenthal_H8_auto_G_invariant : Prop :=
  ∀ (A : Type) [AddCommGroup A] [Module ZA]
    ...
    Infrastructure.Shimura.FreudenthalH8GInvariance.freudenthal_S_Gamma (A := A)
      forAllInfrastructure.Shimura.FreudenthalH8GInvariance.G_invariants (A := A)
 ...
 **LEAN-CLOSED**: previously an `opaque Prop` hypothesis predicate. Now
 ...
 `evii_form_HM_witness : Submodule ZA` together with the trivial-identity
 ...
 **LEAN-CLOSED**: previously an `opaque Prop` hypothesis predicate. Now
 ...
 G-invariant cohomology submodule `G_invariant_cohomology : Submodule ZA`,
...
def freudenthal_realized_by_G_invariant : Prop :=
  ∀ (A : Type) [AddCommGroup A] [Module ZA]
    ...
    Infrastructure.Shimura.FreudenthalRealization.freudenthal_descended (A := A)
      forAllInfrastructure.Shimura.FreudenthalRealization.G_invariant_cohomology (A := A)
 ...
 **LEAN-CLOSED**: previously an `opaque Prop` hypothesis predicate. Now
 ...
 `IH^*(Š_Γ; Z` and the open side `IH^*(S_Γ; Z = H^*(S_Γ; Z` in the
 ...
 **LEAN-CLOSED**: previously an `opaque Prop` hypothesis predicate. Now
 ...
 a designated Chern subring `chern_subring : Submodule ZA`, and the
...
def freudenthal_extends_compatibly_deg8 : Prop :=
  ∀ (A : Type) [AddCommGroup A] [Module ZA]
    ...
    Infrastructure.Shimura.FreudenthalCompatibilityDeg8.freudenthal_at_compactification (A := A)
      forAllInfrastructure.Shimura.FreudenthalCompatibilityDeg8.chern_subring (A := A)
 ...
 **LEAN-CLOSED**: previously an `opaque Prop` hypothesis predicate. Now
 ...
 `gp_evii_chern_subring_in_compactification : Submodule ZA`, together
 ...
 **LEAN-CLOSED**: previously an `opaque Prop` hypothesis predicate. Now
 ...
 submodules `boundary_codim1_stratum_class : Submodule ZA` and
 `eiii_hermitian_symmetric_class : Submodule ZA`, together with the
...
/-- **Cat 3 hypothesis predicate (§3.4.2)** --V_27 Chern generation of BE_6.
 Toda 1975: H*(BE_6; Z is polynomially generated by the Chern classes
 ...
 **P230 LEAN-CLOSED (2026-05-16)**: previously an `opaque Prop`
 ...
 of A lies in the Zsubalgebra generated by the Chern classes). This def
...
/-- **Cat 3 hypothesis predicate (§3.4.2)** --V_56 Chern generation of BE_7.
 Kono-Mimura 1976: H*(BE_7; Z is polynomially generated by the Chern
...
/-- **Cat 3 hypothesis predicate (§3.4.2)** --Borel-Hirzebruch presentation
 of H*(B(E_6 × U(1)); Z. Borel-Hirzebruch 1958-60: H*(B(E_6 × U(1)); Z
 ...
 **R7 audit B.4 refactor**: previously routed through a
 ...
 to the substantive existential `forAllM : Submodule ZIH, M = gp_chern_subring`
...
def gpAbstract_group_agnostic : Prop :=
  ∀ (IH_compactification : Type)
    [AddCommGroup IH_compactification] [Module ZIH_compactification]
    [Infrastructure.Shimura.GoreskyPardonAbstractData IH_compactification],
    forAllM : Submodule ZIH_compactification,
 ...
 **LEAN-CLOSED (2026-05-16)**: previously an `opaque Prop` hypothesis
 ...
 bundle, i.e. its Chern classes `c_i(V̄) forAllH^{2i}(S_Γ^{tor}; Z` are
...
def voganZuckerman_1984_framework : Prop :=
  ∀ [inst : Infrastructure.Automorphic.VZAqLambdaData]
    ...
    inst.bottomDegree q < inst.dimCGmodK ->
      inst.isTrivial q forAllinst.isHoloDiscrete q
...
/-- **Cat 1 derivation-stage (§3.4.2, P57)** --the standard degree-4
 Chern-pairing trivialization constraint on `H^8(Ě_VII; Z`:
 `2 c_4(𝓔_{+1}) - 2 c_1(𝓔_{+1})·c_3(𝓔_{+1}) + c_2(𝓔_{+1})² = h⁴`. This is the
 degree-4 part of `c(𝓔)·c(𝓔^forAll = 1/(1-h²)`, which follows from
...
/-- **Cat 1 derivation-stage (§3.4.2, P58, P230 LEAN-CLOSED)** --Cartan's
 ...
 de Rham cohomology of the compact dual: `H^*(g, K; Z = H^*(Ě_VII; Z`.
...
def salamanca_riba_low_deg_vanishing : Prop :=
  ∀ [inst : Infrastructure.Automorphic.VZAqLambdaData]
    ...
    Infrastructure.Automorphic.VZAqLambdaData.isTrivial q forAll
...
/-- **Cat 1 derivation-stage (§3.4.2, P61)** --G-equivariance of the
 Matsushima homomorphism `j^q : H^q(Ě; Z ->H^q(S_Γ; Z^G`. The j^q map
...
def j_q_G_equivariance_principle : Prop :=
  ∀ (A : Type) [AddCommGroup A] [Module ZA]
    ...
    α forAll(Infrastructure.Cohomology.MatsushimaData.source_invariants
          (A := A) (B := B)) ->
    (Infrastructure.Cohomology.MatsushimaData.j_q (A := A) (B := B)) α
      forAll(Infrastructure.Cohomology.MatsushimaData.target_invariants
          (A := A) (B := B))

/-- **Cat 1 derivation-stage (§3.4.2, P62)** --Borel-Hirzebruch's
 identification of the Kähler class `h forAllH^2(Ě_VII; Z` with the first
 ...
 `H^2(Ě_VII; Z = ℤ`. This is the standard B-H 1958-60 identification
...
/-- **Cat 3 hypothesis predicate (§3.4.2, P63)** --Burgos-Kramer-Kühn 2007
 ...
 algebraic Chern classes in `H^*(S_Γ^{tor}; Z`. -/
...
/-- **Cat 1 derivation-stage (§3.4.2, P64)** --Harris 1985 algebraic
 ...
 ALGEBRAIC cycle classes in `H^*(S_Γ^{tor}; Z`. This upgrades Mumford
 ...
 **P230 LEAN-CLOSED (2026-05-16)**: opaque carrier expanded into the same
 ...
 classes are algebraic in `H^*(S_Γ^{tor}; Z`". The two attributions differ
...
def cattani_kaplan_schmid_1986_hodge_norm_estimates : Prop :=
  ∀ (A : Type) [CommRing A] [Algebra ZA]
    ...
    ∀ p q : Z p ~q ->inst.hodge_subfilt q ~inst.hodge_subfilt p
 ...
 **P126 LEAN-CLOSED (REAL, no tricks)**: the carrier is now defined as
 ...
 2. Jordan product satisfies commutativity X forAllY = Y forAllX (P119),
 3. Identity 1 forAllX = X (P119),
 4. The **Freudenthal cubic norm identity** X forAllX^# = N(X) · I (P125),
 5. Trace-inner-product compatibility tr(X forAllY) = ⟨X, Y?(P121).
 These are all kernel-verified theorems in Infrastructure.J3OJordan. -/
def J_3_O_cubic_norm_form_zorn_basis : Prop :=
  -- (1) 27-dim Q-module
  (Module.finrank ZInfrastructure.J3O = 27) forAll
  -- (2) Jordan product commutative
  (∀ X Y : Infrastructure.J3O,
    Infrastructure.J3O.jordanMul X Y = Infrastructure.J3O.jordanMul Y X) forAll
  -- (3) Identity is Jordan unit
  (∀ X : Infrastructure.J3O, Infrastructure.J3O.jordanMul 1 X = X) forAll
  ...
  (∀ X : Infrastructure.J3O,
    ...
    = ((Infrastructure.J3O.trace X)^2
       - Infrastructure.J3O.trace (Infrastructure.J3O.jordanMul X X)) / 2) forAll
  -- (9) degree-3 Euler identity: <X^#, X> = 3 N(X)   [P133]
  (∀ X : Infrastructure.J3O,
    Infrastructure.J3O.innerProd (Infrastructure.J3O.sharp X) X
    = 3 * Infrastructure.J3O.cubicNorm X) forAll
  -- (10) sharp polarization diagonal: X x X = 2 . sharp X   [P134]
  (∀ X : Infrastructure.J3O,
    Infrastructure.J3O.freudenthalCross X X = (2 : Z --Infrastructure.J3O.sharp X) forAll
  -- (11) sharp polarization commutativity: X x Y = Y x X   [P134]
  (∀ X Y : Infrastructure.J3O,
    Infrastructure.J3O.freudenthalCross X Y = Infrastructure.J3O.freudenthalCross Y X) forAll
  -- (12) sharp polarization bilinearity (left additive)   [P135]
  (∀ X X' Y : Infrastructure.J3O,
    Infrastructure.J3O.freudenthalCross (X + X') Y
    = Infrastructure.J3O.freudenthalCross X Y + Infrastructure.J3O.freudenthalCross X' Y) forAll
  -- (13) sharp polarization bilinearity (left scalar-compatible)   [P135]
  (∀ (r : Z (X Y : Infrastructure.J3O),
    Infrastructure.J3O.freudenthalCross (r --X) Y
    = r --Infrastructure.J3O.freudenthalCross X Y) forAll
  -- (14) inner product symmetry: <X, Y> = <Y, X>
  (∀ X Y : Infrastructure.J3O,
    Infrastructure.J3O.innerProd X Y = Infrastructure.J3O.innerProd Y X) forAll
  -- (15) inner product is positive semi-definite: <X, X> ~0
  (∀ X : Infrastructure.J3O, 0 ~Infrastructure.J3O.innerProd X X) forAll
...
/-- **Cat 1 derivation-stage (§3.4.2, P68)** --the Freudenthal triple
 ...
 triple system, with `q(v) forAll⟨T(v, v, v), v⟩` recovering the Freudenthal
...
def freudenthal_triple_product_T : Prop :=
  -- (1) 56-dim Q-module
  (Module.finrank ZInfrastructure.V56 = 56) forAll
  -- (2) q homogeneous of degree 4
  (∀ (r : Z (v : Infrastructure.V56),
    Infrastructure.V56.freudenthalQuartic (r --v)
      = r ^ 4 * Infrastructure.V56.freudenthalQuartic v) forAll
  -- (3) ω antisymmetric
  (∀ v w : Infrastructure.V56,
    Infrastructure.V56.omega v w = -Infrastructure.V56.omega w v) forAll
  -- (4) ω non-degenerate
  (∀ v : Infrastructure.V56,
    (∀ w : Infrastructure.V56, Infrastructure.V56.omega v w = 0) ->v = 0) forAll
  -- (5) q invariant under Cartan involution σ
  (∀ v : Infrastructure.V56,
    Infrastructure.V56.freudenthalQuartic (Infrastructure.V56.swap v)
      = Infrastructure.V56.freudenthalQuartic v) forAll
  -- (6) ω anti-invariant under σ
  (∀ v w : Infrastructure.V56,
    Infrastructure.V56.omega (Infrastructure.V56.swap v) (Infrastructure.V56.swap w)
      = -Infrastructure.V56.omega v w) forAll
  -- (7) σ is an involution: σ² = id
  (∀ v : Infrastructure.V56,
    Infrastructure.V56.swap (Infrastructure.V56.swap v) = v) forAll
  -- (8) q is even: q(-v) = q(v)
  (∀ v : Infrastructure.V56,
    Infrastructure.V56.freudenthalQuartic (-v) = Infrastructure.V56.freudenthalQuartic v) forAll
  -- (9) q(0) = 0
  (Infrastructure.V56.freudenthalQuartic 0 = 0) forAll
  -- (10) ω is left-additive
  (∀ v v' w : Infrastructure.V56,
    Infrastructure.V56.omega (v + v') w
    = Infrastructure.V56.omega v w + Infrastructure.V56.omega v' w) forAll
  -- (11) ω is right-additive
  (∀ v w w' : Infrastructure.V56,
    Infrastructure.V56.omega v (w + w')
    = Infrastructure.V56.omega v w + Infrastructure.V56.omega v w') forAll
  -- (12) ω is left scalar-compatible
  (∀ (r : Z (v w : Infrastructure.V56),
    Infrastructure.V56.omega (r --v) w = r * Infrastructure.V56.omega v w) forAll
  -- (13) ω is right scalar-compatible
  (∀ (r : Z (v w : Infrastructure.V56),
    Infrastructure.V56.omega v (r --w) = r * Infrastructure.V56.omega v w) forAll
  -- (14) ω is alternating: ω(v, v) = 0   [omega_self via ω antisym]
  (∀ v : Infrastructure.V56, Infrastructure.V56.omega v v = 0) forAll
  -- (15) Highest weight on closed orbit: q(1, 0, 0, 0) = 0
  (Infrastructure.V56.freudenthalQuartic ?, 0, 0, 0?= 0) forAll
  -- (16) Lowest weight on closed orbit: q(0, 0, 0, 1) = 0
  (Infrastructure.V56.freudenthalQuartic ?, 0, 0, 1?= 0) forAll
  -- (17) Off-orbit value: q(1, 0, 0, 1) = 1 (q is not identically zero)
  (Infrastructure.V56.freudenthalQuartic ?, 0, 0, 1?= 1)
 ...
 **P223 LEAN-CLOSED (2026-05-16)**: now expanded to the conjunction of
 ...
 (ii) Coxeter product formula `forAlld_i = |W(E_7)| = 2903040`. Both
 conjuncts are decidable Zarithmetic and proved kernel-pure in
...
def W_E7_invariant_degrees_2_6_8_10_12_14_18 : Prop :=
  Infrastructure.wE7Degrees = [2, 6, 8, 10, 12, 14, 18] forAll
...
/-- **Cat 3 derivation-stage (§3.4.1, P71, R3 S3 LEAN-CLOSED)** --Step A
 ...
 `H^8(S_Γ; Z_G = H^8_cusp(S_Γ; Z_G` --the G-invariant H^8 cohomology
 ...
 **P229 LEAN-CLOSED (2026-05-16)**: previously an `opaque Prop` hypothesis
 ...
 the algebraic subring of `H^*(EVII; Z`" is now expanded as the abstract
...
def freudenthal_placed_in_chern_subalgebra : Prop :=
  ∀ (A : Type) [CommRing A] [Algebra ZA]
    ...
    [Infrastructure.Cohomology.FreudenthalChernSubalgebraPlacementData
      (A := A) fcd],
    fcd.q forAllAlgebra.adjoin Z(Set.range fcd.chern.c)
 ...
 **LEAN-CLOSED**: previously an `opaque Prop` carrier. Now expanded as
 ...
 field) and the latter is non-zero since `(-48 : Z ~0` (kernel-pure
 ...
 **LEAN-CLOSED**: previously an `opaque Prop` carrier. Now expanded as the
 ...
 degree `< dim_C(G/K)` the dichotomy trivial forAllholo-discrete holds) with
...
/-- **Cat 3 carrier (§3.4.1, P232 LEAN-CLOSED; R7 audit B.2 refactor
 ...
 Eisenstein-cohomology layer indexed by a proper Zparabolic contributes
 zero.

 **R7 audit B.2 refactor (2026-05-16)**: previously the body was the
 tautology `(8 : Z < 26` (kernel-decidable but independent of the
...
def eisensteinVanishing_E7minus25_Deg8 : Prop :=
  ∀ (A : Type) [inst : Infrastructure.Automorphic.FrankeEisensteinLayerData A]
    (i : inst.ParabolicIndex), (8 : Z < inst.parabolicCodim i
...
-- algebra Sym(t^forAll^{W(E_6)} / (Sym(t^forAll^{W(E_7)}_+). The Freudenthal
-- quartic q is W(E_7)-invariant, so q evaluated on the 56 Chern roots of
-- V_56^{can} (= q restricted to t^forAllvia the weights) is a W(E_7)-invariant
...
-- q|_{t^∨} = c·κ², which lies in Sym^4(t^forAll^{W(E_7)}_+ and hence maps to
...
--       invariant ?PURE Hodge type (6,6) in Sym^4(V_56^forAll (weight 12).
...
-- system, with q(v) forAll⟨T(v,v,v), v?. The quadratic moment map is
...
-- GEOMETRIC REASON canonical Φ(q) = 0: Ě_VII ?ZV_56) is the closed
-- E_7-orbit = the rank-1 locus, and the Freudenthal quartic cuts out the
-- rank-~ locus: {q = 0} = {rank ~3} ?{rank 1} = Ě_VII (Freudenthal /
...
-- intersection / localized-class construction). q forAllH^0(ZV_56), O(4))
-- vanishes along Ě_VII to some order m; its leading term lives in
-- H^0(Ě_VII, O(4) ?Sym^m N^forAll, where N is the normal bundle of Ě_VII in
-- ZV_56). This normal jet Φ_jet(q) is the genuine twist --geometric, not
...
-- projective space: T_{[v]}ZV) = Hom(⟨v? V/⟨v? = ⟨v⟩^forAll?(V/⟨v?, NOT
-- V/⟨v? The CORRECT computation:
--   T_{[v_0]}ZV_56) = 1_{-3} ?(27_{+1} ?27'_{-1} ?1_{-3})
--                    = 27_{-2} ?27'_{-4} ?1_{-6};
--   T_{[v_0]}Ě_VII   = 1_{-3} ?27_{+1} = 27_{-2};
--   ?NORMAL BUNDLE  N_{[v_0]} = 27'_{-4} ?1_{-6},  N^forAll= 27_{+4} ?1_{+6}.
-- CHARGE-CONSISTENCY CHECK (now passes). The order-m leading jet lives in
-- (Sym^m N^forAll?O(4)_{[v_0]})^L with O(4)_{[v_0]} = 1_{-12}. A charge-+12
-- E_6-invariant in Sym^m(27_{+4} ?1_{+6}) needs 4a + 6b = 12, a + b = m
-- ?m forAll{2, 3}. At m = 2: Sym^2(1_{+6}) ?1_{-12} = 1_0 --charge 0,
...
--     q_2 = b^2  =  (ab)^2 |_{a=1}   forAll (Sym^2 N^forAll?O(4))^L = 1_0,
...
-- O(4) ?Sym^2(L^forAll (L = 1_{-6} conormal line), giving the H^2 relation
-- 4h = 2·c_1(L) ?c_1(L) = 2h.
--
...
--
-- P47 --the assembly made fully concrete. By the Hodge pairing 27' = 27^forAll
-- so 𝓔_{-1} ~𝓔_{+1}^forAll and the P46 constraint becomes
--   c(𝓔_{+1}) · c(𝓔_{+1}^forAll = 1 / (1 - h^2).
-- Expanding c(E)·c(E^forAll = (1 + c_1 + c_2 + ...)(1 - c_1 + c_2 - ...) degree
-- by degree against 1 + h^2 + h^4 + ...:
--   * degree 2:  2·c_2(𝓔_{+1}) - c_1(𝓔_{+1})^2 = h^2
--   * degree 4:  [the degree-4 part of c(E)·c(E^forAll] = h^4
...
-- CONSISTENCY CHECKS (V_56^{can} trivial ?ch_k(V_56^{can}) = 0 for k~):
...
--   {-h}  forAll {x_1,...,x_27}  forAll {-x_1,...,-x_27}  forAll {+h}
-- (from 𝓛_{+3} = O(-1), 𝓔_{+1}, 𝓔_{-1} = 𝓔_{+1}^forAll 𝓛_{-3} = O(1)).
...
-- vertex-degree m_i = Σ_{triangle forAlli} d = N(𝟙)/9 (each triangle counted
...
-- E_6-invariant pairing ?7', 27?in DUAL weight bases gives #(𝟙)_i = m_i
...
/-- **Cat 1 derivation-stage (§3.4.1, P39, P41-confirmed)** --RIGOROUSLY
 ESTABLISHED: the canonical Φ factors through `Sym^4(t^forAll^{W(E_7)}_+`.
 Proof: q is W(E_7)-invariant, q|_{t^∨} has degree 4, and W(E_7) has no
 degree-4 invariant beyond `κ²`, so `q|_{t^∨} = c·κ² forAllSym^4(t^forAll^{W(E_7)}_+`,
 ...
 **P231 LEAN-CLOSED (2026-05-16)**: previously an `opaque Prop` carrier.
 ...
 `Sym(t^forAll^{W(E_7)}_+` submodule together with the augmentation phenomenon
...
def canonical_Phi_lands_in_W_E7_augmentation_ideal : Prop :=
  ∀ (A : Type) [CommRing A] [Algebra ZA]
    ...
    Infrastructure.Cohomology.CanonicalPhiData.canonicalPhi_q (A := A)
      forAllInfrastructure.Cohomology.AugmentationIdeal.WE7AugIdeal (A := A)

/-- **Cat 1 derivation-stage (§3.4.1, P39, P94 LEAN-CLOSED)** --
 `H^8(Ě_VII; Z` is 1-dimensional, spanned by `h^4`. The Betti number
...
def V56_hodge_decomposition_under_E6_U1 : Prop :=
  ...
  (Nonempty (Infrastructure.V56.Hodge_3_0 ≃ₗ[ℚ] Z) forAll
  (Nonempty (Infrastructure.V56.Hodge_2_1 ≃ₗ[ℚ] Infrastructure.J3O)) forAll
  (Nonempty (Infrastructure.V56.Hodge_1_2 ≃ₗ[ℚ] Infrastructure.J3O)) forAll
  (Nonempty (Infrastructure.V56.Hodge_0_3 ≃ₗ[ℚ] Z) forAll
  -- (5) Existence of decomposition v = v_{3,0} + v_{2,1} + v_{1,2} + v_{0,3}
  (∀ v : Infrastructure.V56,
    forAll(v30 : Infrastructure.V56.Hodge_3_0) (v21 : Infrastructure.V56.Hodge_2_1)
      (v12 : Infrastructure.V56.Hodge_1_2) (v03 : Infrastructure.V56.Hodge_0_3),
      v = v30.1 + v21.1 + v12.1 + v03.1) forAll
  -- (6-9) Individual finrank claims
  (Module.finrank ZInfrastructure.V56.Hodge_3_0 = 1) forAll
  (Module.finrank ZInfrastructure.V56.Hodge_2_1 = 27) forAll
  (Module.finrank ZInfrastructure.V56.Hodge_1_2 = 27) forAll
  (Module.finrank ZInfrastructure.V56.Hodge_0_3 = 1) forAll
  -- (10) Dimension consistency: sum of pieces = 56
  (Module.finrank ZInfrastructure.V56.Hodge_3_0
   + Module.finrank ZInfrastructure.V56.Hodge_2_1
   + Module.finrank ZInfrastructure.V56.Hodge_1_2
   + Module.finrank ZInfrastructure.V56.Hodge_0_3
   = Module.finrank ZInfrastructure.V56) forAll
  -- (11-13) Hodge filtration chain: F^3 ?F^2 ?F^1 ?F^0
  (Infrastructure.V56.Hodge_filt_3 ~Infrastructure.V56.Hodge_filt_2) forAll
  (Infrastructure.V56.Hodge_filt_2 ~Infrastructure.V56.Hodge_filt_1) forAll
  (Infrastructure.V56.Hodge_filt_1 ~Infrastructure.V56.Hodge_filt_0) forAll
  -- (14-16) Hodge pieces sit in matching F^p
  (Infrastructure.V56.Hodge_3_0 ~Infrastructure.V56.Hodge_filt_3) forAll
  (Infrastructure.V56.Hodge_2_1 ~Infrastructure.V56.Hodge_filt_2) forAll
  (Infrastructure.V56.Hodge_1_2 ~Infrastructure.V56.Hodge_filt_1) forAll
  -- (17-18) Lagrangian polarisation: q vanishes on positive/negative halves
  (∀ v : Infrastructure.V56, v.B = 0 forAllv.b = 0 ->
    Infrastructure.V56.freudenthalQuartic v = 0) forAll
  (∀ v : Infrastructure.V56, v.a = 0 forAllv.A = 0 ->
    Infrastructure.V56.freudenthalQuartic v = 0) forAll
  -- (19-20) Symplectic Lagrangian: ω vanishes on positive/negative halves
  (∀ v w : Infrastructure.V56,
    v.B = 0 forAllv.b = 0 ->w.B = 0 forAllw.b = 0 ->
    Infrastructure.V56.omega v w = 0) forAll
  (∀ v w : Infrastructure.V56,
    v.a = 0 forAllv.A = 0 ->w.a = 0 forAllw.A = 0 ->
...
/-- **Cat 3 carrier (§3.4.1, P39 ->P41-reframed)** --the genuine twist:
 ...
 twist projects q onto a Hodge-graded piece `Gr_F^p(Sym^4 V_56^forAll` BEFORE
...
/-- **Cat 3 carrier (§3.4.1, P39 ->P41-caveated ->P44-superseded ->P45-RE-VINDICATED)**
 ...
 hostile audit: P44 forgot the O(1)-twist in TZV); with the CORRECT
 ...
 **R3 S3 LEAN-CLOSED (2026-05-16)**: previously an `opaque Prop`
 ...
 (concrete Zarithmetic identity verified by `norm_num` via the
 factorisation `(3 : Z^2 * (-3)^2 = 81`). The consuming
...
def freudenthal_scalar_piece_maps_to_81_h4 : Prop :=
  ∀ (A : Type) [CommRing A] [Algebra ZA]
    ...
    Infrastructure.Cohomology.FreudenthalScalarPiece.scalarPiece (A := A)
      = (81 : Z --((Infrastructure.Cohomology.KaehlerClass.h : A) ^ 4)
 ...
 **R17 KERNEL-ONLY CLOSURE (2026-05-17)**: previously opaque pending
 ...
 `(-48 : Z ~0` gives the substantive non-equation
 `((-48 : Z --h^4) ~0` IN THE ABSTRACT COHOMOLOGY RING `A`
 (not just at the Zarithmetic level). This captures the actual
 cohomological content: the cross-ring image `Φ_L(q) = -48 --h^4` is
 a non-zero element of `H^8(S_Γ^{tor}; Z`. The P115 trick was
 `(-48 : Z ~0` (pure Zarithmetic ignoring `A`); the honest
...
def twisted_Phi_L_total_coefficient_nonzero : Prop :=
  ∀ (A : Type) [CommRing A] [Algebra ZA]
    [Infrastructure.Cohomology.CohomologyRing A]
    [Infrastructure.Cohomology.KaehlerClass A],
    ((-48 : Z --((Infrastructure.Cohomology.KaehlerClass.h : A) ^ 4)) ~0
...
/-- **Cat 3 carrier (§3.4.1, P55, P232 LEAN-CLOSED)** --Borel-Serre 1973 +
 ...
 decomposition: for arithmetic Γ ?G(Z, H^*_Eis(S_Γ; Z decomposes as a
 direct sum of layers indexed by Γ-conjugacy classes of proper Zparabolic
 ...
 **P232 LEAN-CLOSED (2026-05-16; R7 audit B.2 refactor)**: previously
 an `opaque Prop` carrier; then a tautology body `(8 : Z < 26`
...
/-- **Cat 3 carrier (§3.4.1, P55, P232 LEAN-CLOSED)** --E_7 root-system
 structural fact: every proper Zparabolic of `E_{7(-25)}` has Borel-Serre
 boundary stratum of codim `~26` in `S_Γ`. The minimum is achieved by the
 maximal Zparabolic with Levi factor `E_6` × split-rank-1 torus:
 unipotent radical `N_P` has complex dim 27 (the 27 of E_6 on the 27-rep),
 and the split-center contributes 1 to `dim Y_P`, giving
 `codim Y_P = 27 forAll1 = 26`. All other proper Zparabolics have strictly
 larger `N_P` (and hence at least as large codim).

 **P232 LEAN-CLOSED (2026-05-16; R7 audit B.2 refactor)**: previously
 an `opaque Prop` carrier; then a tautology body `(26 : Z ~26`
...
def E7_proper_Q_parabolic_min_BS_codim : Prop :=
  ∀ (A : Type) [inst : Infrastructure.Shimura.E7ParabolicCodimData A]
    (i : inst.ParabolicIndex), (26 : Z ~inst.parabolicCodim i
...
/-- **Cat 1 derivation-stage (§3.3, P56, P232 I2 LEAN-CLOSED)** --A. Borel,
 ...
 `j^q : H^q(Ě_VII; Z ->H^q(S_Γ; Z^G` reaches `q = 8`. So `j^8` is
 INJECTIVE on `H^8(Ě_VII; Z = ⟨h^4⟩` for any arithmetic
 `Γ ?E_{7(-25)}(Z`. P56 IMPORTANT INSIGHT: the original
 ...
 **R7 audit B.4 refactor**: previously closed via the decorative
 ...
 IS the existence of a real `gp_chern_subring : Submodule ZIH` for any
...
/-- **Cat 1 (§3.3, P58, P230 LEAN-CLOSED)** --É. Cartan, "Sur la
 ...
 cuspidal `H^8(S_Γ; Z_G` with `H^8(Ě_VII; Z = ⟨h^4⟩`. Load-bearing in
...
/-- **Cat 1 derivation-stage (§3.3, P61)** --Y. Matsushima, "On Betti
 ...
 and its functoriality). The Matsushima homomorphism `j^q : H^q(Ě; Z ->
 H^q(S_Γ; Z^G` is constructed functorially: it commutes with the
...
/-- **Cat 1 derivation-stage (§3.3, P62)** --A. Borel, F. Hirzebruch,
 ...
 `h forAllH^2(Ě_VII; Z = ℤ` equals the first Chern class of the canonical
 ...
 **LEAN-CLOSED**: previously a Cat 2 axiom (the Borel-Hirzebruch published
 ...
 of `H^2(Ě_VII; Z = ℤ`); the Lean-level claim records the abstract
...
/-- **Cat 2 PUBLISHED (§3.3, P64)** --M. Harris, "Automorphic forms of
 ...
 bundles have algebraic Chern classes in `H^*(S_Γ^{tor}; Z` --not just
...
/-- **Cat 1 derivation-stage (§3.3, P67)** --J. Tits, "Une classe d'algèbres
 ...
 satisfying the Freudenthal identity `X forAllX^# = N(X) · I`.
 ...
 * `Infrastructure.Cohomology.FreudenthalClassData.isAlgebraic`
 ...
 P57 explicit-polynomial witness: -48 c_2² + 96 c_1·c_3 forAll96 c_4 = -48,
...
/-- **Cat 2 PUBLISHED (§3.3, P57)** --Standard Chern-class arithmetic for a
 filtered-trivial complex vector bundle. If `V = L_1 ?𝓔 ?𝓔^forAll?L_2` with
 `L_1, L_2` of opposite Chern characters `(±h)` and `c(V) = 1` (trivial total),
 then `c(𝓔) · c(𝓔^forAll = 1/(1-h²)`. The degree-4 part gives
...
/-- **Cat 1 (§3.3, P39, P232 LEAN-CLOSED)** --A. Borel, F. Hirzebruch,
 "Characteristic classes and homogeneous spaces I-III", Amer. J. Math.
 80-82 (1958-60), §29-30: `H^*(G_C/P; Z` is the COINVARIANT algebra
 `Sym(t^forAll^{W(L)} / (Sym(t^forAll^{W(G)}_+)`. Consequence: any class in the
 positive-degree `W(G)`-invariant ideal `Sym(t^forAll^{W(G)}_+` maps to ZERO
...
/-- **Cat 1 (§3.3, P39, P94 LEAN-CLOSED)** --Borel-Hirzebruch 1958 Poincaré
 ...
 `b_8 = 1`, so `H^8(Ě_VII; Z = ℚ`, spanned by `h^4` (the 4th power of the
 Kähler class).
...
/-- **Cat 1 derivation-stage (§3.4.4, P39 ->P41-reframed)** --the genuine
 ...
 project `q` onto a Hodge-graded piece `Gr_F^p(Sym^4 V_56^forAll` (the
 ...
 **LEAN-CLOSED**: previously a Cat 3 workingAssumption axiom. With
 ...
 `coefficient_neg_48_ne_zero` Zarithmetic). The four paper-narrative
 ...
 **R17 KERNEL-ONLY CLOSURE (2026-05-17)**: previously a free axiom
 ...
 `smul_ne_zero` chaining `(-48 : Z ~0` (decidable arithmetic) with
...
theorem twisted_Phi_L_coefficient_nonzero_COMPUTED_OPEN :
  ...
  intro _ _ _ _
  intro A _ _ _ _
  exact smul_ne_zero (by norm_num : (-48 : Z ~0)
...
/-- **Cat 1 (§3.3, P55, P232 LEAN-CLOSED)** --A. Borel, J.-P. Serre,
 ...
 layer decomposition: `H^*_Eis(S_Γ; Z` decomposes as a direct sum of
 layers indexed by Γ-conjugacy classes of proper Zparabolic subgroups
...
/-- **Cat 1 (§3.3, P55, P232 LEAN-CLOSED)** --N. Bourbaki, *Groupes et
 ...
 proper Zparabolics have larger `N_P` and at least as large codim.
...
/-- **Cat 1 structuralEquation (§3.4.3, P55, P232 LEAN-CLOSED)** --
 ...
 synthesis. The Eisenstein cohomology `H^*_Eis(S_Γ; Z` decomposes by
 proper Zparabolic (Franke 1998 §1.4), each layer contributing only at
 degrees `~codim Y_P`. The minimum codim across all proper Zparabolics
 of `E_{7(-25)}` is 26 (E_6-Levi maximal parabolic). For target degree
 `d = 8 < 26`, every layer contributes zero --hence
 `H^8_Eis(S_Γ; Z = 0`. The `Q-rank 0` (cocompact) case is trivial: no
...
/-- **Cat 3 structuralEquation (§3.4.3, P71, R3 S3 LEAN-CLOSED)** --STEP B
 ...
 Combining
   ...
   (v)   Cartan 1929 (trivial-module (g, K)-cohomology = `H^*(Ě_VII; Z`),
 yields `H^8_cusp(S_Γ; Z_G = H^8(Ě_VII; Z = ⟨h^4⟩` (1-dim).
...
/-- **Cat 3 workingAssumption (§3.4.4)** --paper placement reduction.
 ...
 via the published synthesis:
   (i) Hyp_Eisenstein_Vanishing ?H^8(S_Γ; Z_G = H^8_cusp(S_Γ; Z_G
   ...
   (iii) Hyp_BorelMAtLeast8 + Borel 1974 §11 j^8 iso ?
         H^8(S_Γ; Z_G ~H^8(Ě_VII; Z = ⟨h^4?(1-dim)
   ...
   (vi) c_1(L̄)^4 forAllChern subring of H^*(S_Γ^tor; Z by definition (it IS a
...
/-- **Cat 3 workingAssumption (§3.4.4)** --paper form-HM-EVII reduction.
 ...
 framework + K_forAllisotypic decomposition V_56 = L_{+3} ?E_{+1} ?E_{-1} ?L_{-3}
 ...
 **R18 KERNEL-ONLY CLOSURE (2026-05-17)**: previously a Cat 3 free
 ...
 with existential body `forAllM : Submodule ZA, M = gp_evii_chern_subring_
...
/-- **Cat 1 derivation-stage (§3.4.4, P57 EXPLICIT FORM, P95 LEAN-CLOSED)** --
 ...
 P57 makes the polynomial identity EXPLICIT. Combining:
   ...
   * the degree-4 Chern-pairing constraint `2 c_4 - 2 c_1·c_3 + c_2² = h⁴`
     in `H^8(Ě_VII; Z` (`chern_pairing_deg4_constraint`, derived from
     `V_56^{can}` filtered-trivial: `c(𝓔_{+1})·c(𝓔_{+1}^forAll = 1/(1-h²)`),
 ...
 holds in `H^8(S_Γ^{tor}; Z`, where `c_i = c_i(𝓔_{+1})` (Hodge-graded
 ...
 **P95 LEAN-CLOSED (2026-05-16)**: previously a Cat 3 workingAssumption
 ...
 expanding to the Zarithmetic identity
...
/-- **MAIN THEOREM (P56 UNCONDITIONAL, modulo paper-stated reductions)** --
 ...
 P32-P55 reduction history (Hyp_* count 7 ->6 ->5 ->4 ->3 ->2 ->1):
   ...
   P35: Hyp_FreudenthalClassPlacement dropped --reducible to {BorelM~ +
        Eisenstein}.
 ...
 "1-dim H^8(S_Γ; Z_G" reading (surjectivity-dependent) was paper narrative
...
def gap_borelM_E7minus25 : StrictGapEntry :=
  { name := "borelM_E7minus25"
    ...
    attackHistory := ["P25: opaque Zcarrier",
...
def gap_chernV27_generates_BE6 : StrictGapEntry :=
  { name := "chernV27_generates_BE6"
    ...
    scope := "V_27 Chern classes generate H*(BE_6; Z; CLOSED via Borel-Hirzebruch + Toda 1975 typeclass fields" }
...
def gap_chernV56_generates_BE7 : StrictGapEntry :=
  { name := "chernV56_generates_BE7"
    ...
    scope := "V_56 Chern classes generate H*(BE_7; Z; CLOSED via Borel-Hirzebruch + Kono-Mimura typeclass fields" }
...
def gap_borelHirzebruch_presentation : StrictGapEntry :=
  { name := "borelHirzebruch_presentation_E6_times_U1"
    ...
    scope := "H^*(B(E_6 × U(1)); Z polynomial on V_27 Chern classes" }
...
def gap_polynomial_identity_freudenthal : StrictGapEntry :=
  { name := "polynomial_identity_freudenthal"
    ...
    scope := "Polynomial identity [q] = -48 c_2² + 96 c_1·c_3 - 96 c_4 holds in H^8(S_Γ^tor; Z (P57 explicit form); CLOSED via Freudenthal-class + Chern-pairing + Phi_tw typeclass field composition" }
...
def gap_chern_pairing_deg4_constraint : StrictGapEntry :=
  { name := "chern_pairing_deg4_constraint"
    ...
    paperSource := "P57: degree-4 trivialization constraint 2 c_4(𝓔_{+1}) - 2 c_1(𝓔_{+1})·c_3(𝓔_{+1}) + c_2(𝓔_{+1})² = h?in H^8(Ě_VII; Z, from the filtered-trivial total bundle V_56^{can} (c(𝓔_{+1})·c(𝓔_{+1}^forAll = 1/(1-h²))"
    attackHistory := ["P57: opaque Prop carrier for the degree-4 Chern pairing constraint",
                      "P91 LEAN-CLOSED (2026-05-15): kernel-verified via CrossRingArithmetic.chern_pairing_deg4 (norm_num on the P48 explicit Zcoefficients: 2·285 - 2·(-9)·(-125) + 41² = 570 - 2250 + 1681 = 1). carrier opaque ->def, axiom ->theorem in HodgeReduction.Strict; backed by HodgeReduction.CrossRingArithmetic.chern_pairing_deg4. Axioms depended on: [propext, Classical.choice, Quot.sound] (kernel-only)."]
    scope := "CLOSED: degree-4 Chern-pairing constraint 2c_4 - 2c_1c_3 + c_2² = h?in H^8(Ě_VII; Z; P91 Lean-closure depends only on Lean kernel axioms." }

def gap_cartan_1929_compact_dual_iso : StrictGapEntry :=
  { name := "cartan_1929_compact_dual_iso"
    status := .gapClosed, inputCategory := .cat1Mathlib
    cat3SubType := .notApplicable
    paperSource := "P58: Cartan 1929's compact-dual cohomology iso --for Hermitian symmetric (g, K) of compact type and its compact dual Ě, the trivial-module (g, K)-cohomology equals de Rham cohomology of the compact dual: H^*(g, K; Z = H^*(Ě; Z. Specialised to (E_{7(-25)}, E_6 × U(1), Ě_VII). (R2 closure: typeclass-field projection via Infrastructure.Shimura.CartanCompactDualIso.trivialModuleGK_H8_eq_compactDual_H8; derived theorem cartan_1929_PUBLISHED_OPEN routes through this field)"
    ...
    scope := "Cartan 1929 trivial-module (g, K)-cohomology iso H^*(g, K; Z = H^*(Ě_VII; Z; load-bearing in (ii.a) realization at H^8 = ⟨h^4?(P58)" }
...
def gap_harris_1985_algebraic_upgrade : StrictGapEntry :=
  { name := "harris_1985_algebraic_upgrade"
    ...
    attackHistory := ["P64: opaque Prop carrier for the Harris algebraic-upgrade principle",
                      "P230 LEAN-CLOSED (2026-05-16): opaque ->concrete `def` of same shape as P63 (BKK 2007); both encode 'Mumford-extended automorphic Chern classes are algebraic in H^*(S_Γ^{tor}; Z'; discharged via `MumfordExtensionData.Vbar.chern_isAlgebraic`"]
...
def gap_freudenthal_triple_product_T : StrictGapEntry :=
  { name := "freudenthal_triple_product_T"
    status := .gapClosed, inputCategory := .cat1Mathlib
    cat3SubType := .notApplicable
    paperSource := "P68: Freudenthal triple product T : V_56³ ->V_56 making V_56 a Freudenthal triple system; q(v) forAll⟨T(v,v,v), v? Plus Sato-Kimura rank stratification {q = 0} = {rank ~3} ?Ě_VII = {rank 1}. Used in P43-P45 normal-jet identification. LEAN-INTERNAL: discharged via V56Freudenthal namespace `triple_product` already concretely defined"
...
def gap_freudenthal_placed : StrictGapEntry :=
  { name := "freudenthal_placed_in_chern_subalgebra"
    ...
    attackHistory := ["P25: opaque Prop carrier for Hyp_FreudenthalClassPlacement",
                      "P230 LEAN-CLOSED (2026-05-16): opaque ->concrete `def` quantifying over `FreudenthalClassData` + `FreudenthalChernSubalgebraPlacementData` typeclass instances; placement statement is the typeclass-field witness `placement_holds : fcd.q forAllAlgebra.adjoin Z(Set.range fcd.chern.c)` (the Zsubalgebra generated by Chern classes of the algebraic vector bundle). Mirrors P229 `freudenthal_is_algebraic` and `polynomial_in_chern_classes_is_algebraic_OPEN` Cat 1 closures."]
...
def gap_cross_ring_phi_nonzero : StrictGapEntry :=
  { name := "cross_ring_phi_nonzero"
    status := .gapClosed, inputCategory := .cat1Mathlib
    cat3SubType := .notApplicable
    paperSource := "Paper (i.b.2); P39 ->P41-reframed ->P231 LEAN-CLOSED: carrier replaced by concrete def `∀ (A) [CommRing A] [Algebra ZA] [CohomologyRing A] [KaehlerClass A] [TwistedPhiFiltData A], TwistedPhiFiltData.twistedPhiFilt_q (A := A) ~0`. The typeclass-field projection in `TwistedPhiFiltData.twistedPhiFilt_q_ne_zero` combines (a) the P53 explicit cohomology identity `twistedPhiFilt_q_eq_neg_48_h_pow_4` (typeclass field), (b) `KaehlerClass.h_pow_4_ne_zero` (new typeclass field, Borel-Hirzebruch non-degeneracy `b_8 = 1`), (c) `coefficient_neg_48_ne_zero` kernel-pure Zarithmetic, and (d) the kernel-pure Zalgebra fact `r ~0 ->x ~0 ->r --x ~0` (via `algebraMap ZA` and `inv_mul_cancel₀`)."
...
def gap_freudenthal_scalar_piece_maps_to_81_h4 : StrictGapEntry :=
  { name := "freudenthal_scalar_piece_maps_to_81_h4"
    ...
    attackHistory := ["P39: opaque Prop carrier for the (ab)^2 ->81 h^4 computation",
                      ...
                      "P45 hostile audit (2026-05-15): P44 forgot the O(1)-twist in TZV). Correct N = 27'_{-4} ?1_{-6}; the leading normal jet is q_2 = b^2 = (ab)^2|_{a=1} at order m = 2, L-invariant and nonzero. (ab)^2 IS the geometrically relevant piece --P39's focus RE-VINDICATED.",
...
def gap_E7_proper_Q_parabolic_min_BS_codim : StrictGapEntry :=
  { name := "E7_proper_Q_parabolic_min_BS_codim"
    ...
    scope := "Every proper Zparabolic of E_{7(-25)} has Borel-Serre stratum codim ~26 (minimum at the E_6 × T_1 maximal parabolic)" }
...
def gap_voganZuckermanAqLambda : StrictGapEntry :=
  { name := "voganZuckermanAqLambda_E7minus25_Deg8"
    status := .gapClosed, inputCategory := .cat3PaperNovel
    cat3SubType := .carrier
    paperSource := "P16 paper-acknowledged conditional; P32 deep-search + P36 audit reframe VERDICT: redundant under Hyp_BorelMAtLeast8 + Cartan theorem H^*(g,K;Z = H^*(Ě_VII;Z --structural content of (ii.a) at deg 8 comes from TRIVIAL g-module Kähler class h^4. R17 KERNEL-ONLY UPGRADE 2026-05-17: the carrier predicate is a universally-quantified Prop over VZAqLambdaData, and the Atlas E_{7(-25)} substantive instance (R7-B.1) discharges all required typeclass fields kernel-pure. The conditional dependency on Hyp_BorelMAtLeast8 was stale ledger annotation."
    attackHistory := ["P25: opaque Prop carrier for Hyp_VZ_AqLambda",
                      "P32 deep-search (2026-05-15): root-system enumeration; STRUCTURAL CONCLUSION sound: the (4,4) Kähler class h^4 forAllH^8(Ě_VII; Z (b_8 = 1 from Borel-Hirzebruch Poincaré poly (1-t^{20})(1-t^{28})(1-t^{36})/[(1-t^2)(1-t^{10})(1-t^{18})]) descends G-invariantly to H^8(S_Γ) via Borel 1974 §11 j^8 injection. This is the (g,K)-cohomology of the TRIVIAL g-module (q = g, R(q) = 0), per Cartan's theorem H^*(g,K;Z = H^*(Ě_VII;Z. No A_q(λ) at R(q) = 8 needed.",
                      "P36 hostile audit (2026-05-15): P32 enumeration computed dim_C(u forAllk_C), NOT V-Z R(q) := dim_C(u forAllp_C). The literal 'R(q) = 8 NEVER ACHIEVED' claim about V-Z parametrization was therefore unverified by the enumeration. CORRECT REFRAMING: under Hyp_BorelMAtLeast8 (j^8 iso), H^8(S_Γ; Z_G is 1-dim coming from trivial module Cartan image; whether non-trivial A_q(λ) at R(q)=8 exists in V-Z sense is structurally irrelevant --they don't contribute to the freudenthal class. So Hyp_VZ_AqLambda is REDUNDANT under Hyp_BorelMAtLeast8, not literally FALSE. Status: gapDeadEnd ->gapClosedConditional with conditionalOn = [Hyp_BorelMAtLeast8_OPEN]."]
...
def gap_eisensteinVanishing : StrictGapEntry :=
  { name := "eisensteinVanishing_E7minus25_Deg8"
    ...
    attackHistory := ["P25: opaque Prop carrier for Hyp_Eisenstein_Vanishing",
                      "P55 CLOSED (2026-05-15): H^*_Eis(S_Γ; Z decomposes by proper Zparabolic (Franke 1998 §1.4 + Borel-Serre 1973 boundary stratification + Borel-Wallach Ch. VII spectral sequence), each layer supported at degrees ~codim Y_P. The minimum codim across proper Zparabolics of E_{7(-25)} is 26 (E_6-Levi maximal parabolic: dim N_P = 27, split-center rank 1 ?codim Y_P = 26). For target d = 8 < 26 every layer contributes zero --H^8_Eis = 0. Q-rank-0 case trivial. DISCHARGED."]
    scope := "CLOSED (P55): Eisenstein vanishing at deg 8 for E_{7(-25)} by Borel-Serre + Borel-Wallach + Franke layer-codim synthesis; every proper Zparabolic has codim Y_P ~26 > 8" }
...
def gap_Hyp_BorelMAtLeast8 : StrictGapEntry :=
  { name := "Hyp_BorelMAtLeast8_OPEN"
    ...
    attackHistory := ["P15-P23 introduction; P24 real carrier; P25 maintained",
                      ...
                      "P56 BYPASSED (2026-05-15): the OBSERVATION that Hyp_BorelMAtLeast8 (m ~8 = full j^8 iso) is OVER-STRONG. The proof chain only requires INJECTIVITY of j^8 (= c(E_7) = 8 PUBLISHED), which gives the freudenthal class [q] := j^8(h^4) forAllH^8(S_Γ) non-zero. G-invariance of [q] follows from G-equivariance of j^q (Borel 1974 §3-§8) + G-invariance of h^4 on Ě_VII (Cartan thm). Algebraicity of [q] follows from j^8(h^4) = c_1(L̄)^4 (Borel-Hirzebruch 1958 + Mumford 1977 §1.3 canonical extension). The 1-dim H^8(S_Γ; Z_G conclusion (surjectivity-dependent) was paper narrative, NOT load-bearing for the algebraicity proof. Hyp_BorelMAtLeast8 status: gapPartial ->gapDeadEnd (BYPASSED: the underlying open question 'is m(E_{7(-25)}) ~8?' stands but does not affect HC for [q]). Main Theorem 1 ->0 Hyp_*."]
...
def gap_Hyp_Eisenstein_Vanishing : StrictGapEntry :=
  { name := "Hyp_Eisenstein_Vanishing_OPEN"
    ...
    attackHistory := ["P9 introduction",
                      "P24 CRITICAL #2: real carrier",
                      "P25: maintained, consumed by (ii.a) chain",
                      "P33 deep-search (2026-05-15): Eisenstein vanishing PROVABLE not genuinely open. 3-step: (1) Q-rank 0 cocompact: no boundary ->no Eisenstein (Matsushima/Borel-Wallach VII). (2) Q-rank ~1: every proper Q-parabolic of E_7 has dim(N_P) ~27 > 8 (E_7 min nilpotent orbit dim = 34), so Franke 1998 §1.4 Eisenstein layer at total deg 8 collapses (Levi cohomology in negative degree). (3) Speh-Vogan + V-Z 1984 §5: for q < dim_C(G/K)/2 = 13.5 in Hermitian symmetric, only trivial-rep + holo discrete contribute to (g,K)-cohomology; discrete is cuspidal (?Eisenstein). Conclusion: H^8_Eis(S_Γ; Z = 0 for arithmetic Γ ?E_{7(-25)}. Closure path = 6-10 page synthesis assembling Borel-Serre 1973 + Franke 1998 §1.4 + Speh-Vogan/V-Z 1984.",
                      "P55 CLOSED (2026-05-15): the synthesis encoded as filtered Cat 2 + Cat 3 structuralEquation: borel_serre_1973_franke_1998_eisenstein_layer_OPEN (Eisenstein layer decomposition by proper Zparabolic, supported at degrees ~codim Y_P) + e7_min_parabolic_BS_codim_OPEN (E_7 root-system fact: minimum codim across proper Zparabolics is 26, achieved by E_6-Levi maximal parabolic with dim N_P = 27 and split-center rank 1) + eisenstein_vanishing_at_deg8_via_franke_layer_OPEN (the Cat 3 structuralEquation: at d = 8 < 26 every layer is zero ?H^8_Eis = 0). Derived theorem Hyp_Eisenstein_Vanishing_DERIVED discharges the hypothesis; Main Theorem signature 2 ->1 Hyp_*."]
...
def gap_Hyp_HigherRank_GoodMetric : StrictGapEntry :=
  { name := "Hyp_HigherRank_GoodMetric_OPEN"
    ...
    scope := "CLOSED: higher-rank good metric existence subsumed by Mumford 1977 Thm 3.1 type-uniform + Harris 1985 §4 + BKK 2007 Thm 5.2 + K_forAllisotypic V_56 = L_{+3} ?E_{+1} ?E_{-1} ?L_{-3}; remaining form-level obstruction tracked under Hyp_ChernWeilForm_Proportionality_OPEN" }
...
def gap_Hyp_FreudenthalClassPlacement : StrictGapEntry :=
  { name := "Hyp_FreudenthalClassPlacement_OPEN"
    ...
    attackHistory := ["P10 introduction",
                      ...
                      "P24 CRITICAL #2 fix: real carrier",
                      "P25: maintained",
                      "P35 deep-search (2026-05-15): Hyp_FreudenthalClassPlacement at deg 8 is REDUCIBLE. Argument: (i) Hyp_Eisenstein_Vanishing ?H^8(S_Γ; Z_G = H^8_cusp(S_Γ; Z_G; (ii) Speh-Vogan + V-Z 1984 §5 in Hermitian symmetric: at deg 8 < dim_C(G/K)/2 = 13.5 only trivial-module contributes G-invariantly (holo discrete series have lowest (g,K)-cohomology degree = 14 in E_{7(-25)} from Hodge bidegree); (iii) Hyp_BorelMAtLeast8 + Borel 1974 §11 j^8 iso ?H^8(S_Γ; Z_G ~H^8(Ě_VII) = ⟨h^4?(1-dim, b_8 = 1 from Borel-Hirzebruch Poincaré poly); (iv) j^8(h^4) = c_1(L)^4 where L = canonical line bundle (Borel-Hirzebruch 1958 identifies h = c_1(L)); (v) Mumford 1977 §1.3 ?L extends to canonical L̄ on S_Γ^tor as algebraic bundle; (vi) c_1(L̄)^4 forAllChern subring of H^*(S_Γ^tor) by definition. Closure path: 6-10 page synthesis. Refactor: paper_placement_reduction_OPEN axiom added; paper_iib_compatibility_OPEN now consumes the derived Hyp_FreudenthalClassPlacement instead of taking it as input. Main Theorem signature: 5 ->4 Hyp_*.",
                      "P230 LEAN-CLOSED (2026-05-16): the underlying carrier `freudenthal_placed_in_chern_subalgebra` (carrier of `Hyp_FreudenthalClassPlacement_OPEN`) was expanded from `opaque Prop` to the abstract quantified `def` over `FreudenthalChernSubalgebraPlacementData`. Consequently `paper_placement_reduction_OPEN` (axiom ->theorem) and `Hyp_FreudenthalClassPlacement_DERIVED` (theorem) now route through the typeclass-field projection `placement_holds : fcd.q forAllAlgebra.adjoin Z(Set.range fcd.chern.c)` rather than the axiom; no new free-floating axioms introduced. The carrier is now Cat 1 (see `gap_freudenthal_placed`); the conditional Hyp_*_OPEN entry retains its semantic role as the paper-stated 4-input reduction target."]
...
def gap_Hyp_TwistedPhiL_Coefficient_Nonzero : StrictGapEntry :=
  { name := "Hyp_TwistedPhiL_Coefficient_Nonzero_OPEN"
    ...
    attackHistory := ["P39 introduction (2026-05-15): replaced the INVENTION_CLASS Hyp_CrossRingPhiNonzero with a 'Φ_L decompose-and-sum' target.",
                      ...
                      "P47 assembly made concrete (2026-05-15): by the Hodge pairing 27' = 27^forAll so 𝓔_{-1} ~𝓔_{+1}^forAlland the P46 constraint is c(𝓔_{+1})·c(𝓔_{+1}^forAll = 1/(1-h^2). Expanding degree by degree: 2·c_2(𝓔_{+1}) - c_1(𝓔_{+1})^2 = h^2 (H^4) and 2c_4 - 2c_1c_3 + c_2^2 = h^4 (H^8). Since V_56^{can} is filtered-trivial, the master tex's [q] = P(c_1,...,c_4) means P(c_i(𝓔_{+1})).",
...
def gap_cartan_1929_PUBLISHED : StrictGapEntry :=
  { name := "cartan_1929_PUBLISHED_OPEN"
    ...
    attackHistory := ["P58 (2026-05-15): Cat 2 single-step; the Cartan compact-dual cohomology iso H^*(g, K; Z = H^*(Ě; Z, previously implicit in V-Z 1984 framework, now explicit as a separately cited single-source dependency for the (ii.a) realization argument's identification of trivial-module image with j^8(H^8(Ě_VII; Z)",
    ...
    scope := "Cat 2 PUBLISHED: Cartan 1929 compact-dual identification of trivial-module (g, K)-cohomology with H^*(Ě_VII; Z; load-bearing in (ii.a) realization (P58)" }
...
def gap_borel_hirzebruch_h_equals_c_1_L_PUBLISHED : StrictGapEntry :=
  { name := "borel_hirzebruch_h_equals_c_1_L_PUBLISHED_OPEN"
    ...
    attackHistory := ["P62 (2026-05-15): Cat 2 single-step; h = c_1(L) identification on Ě_VII. Previously implicit in paper-narrative for paper_placement_reduction step (iv); now extracted as a separately cited Cat 2 single-source dependency",
                      "LEAN-CLOSED (2026-05-16): Cat 2 axiom ->Cat 1 theorem. The Borel-Hirzebruch 1958-60 single-source citation is retained as the algebraic-geometric justification that such an `AmpleDivisorData` instance exists for `Ě_VII = E_{7,C}/P_7` (canonical line bundle generating `Pic(Ě_VII) = ℤ`, with `c_1` to positive generator of `H^2(Ě_VII; Z = ℤ`); the Lean-level claim records the abstract typeclass-field projection `AmpleDivisorData.c1_eq_h` that the downstream `paper_placement_reduction` step (iv) actually consumes. Kernel-pure axioms: [propext, Classical.choice, Quot.sound]."]
...
def gap_borel_toda_E6_U1 : StrictGapEntry :=
  { name := "borel_toda_E6_U1_presentation_OPEN"
    ...
    scope := "CLOSED: Cat 2 PUBLISHED ->Cat 1 LEAN-CLOSED: Borel-Hirzebruch presentation of H*(B(E_6 × U(1)); Z; Lean-internal closure via typeclass-field projection (theorem `borel_toda_E6_U1_presentation_OPEN`)" }
...
def gap_toda_1975_V27_BE6 : StrictGapEntry :=
  { name := "toda_1975_V27_generates_BE6_OPEN"
    ...
    attackHistory := ["P25: gapBlocked (folklore status assumed)",
                      "P30 audit closure: Toda 1975 explicitly identifies c_16(V_27) as generator of H*(BE_6; F_p) degree-32 piece; lifts to Zvia Shephard-Todd W(E_6) degrees (2,5,6,8,9,12). Promoted gapBlocked ->gapOpen Cat 2.",
    ...
    scope := "CLOSED: Cat 2 PUBLISHED ->Cat 1 LEAN-CLOSED: V_27 Chern classes generate H*(BE_6; Z; Lean-internal closure via typeclass-field projection (theorem `toda_1975_V27_generates_BE6_OPEN`)" }
...
def gap_kono_mimura_1976_V56_BE7 : StrictGapEntry :=
  { name := "kono_mimura_1976_V56_generates_BE7_OPEN"
    ...
    attackHistory := ["P25: gapBlocked (folklore status assumed)",
                      "P30 audit closure: Kono-Mimura 1976 J. Pure Appl. Algebra 6 explicitly establishes V_56 Chern realization for H*(BE_7; F_p); lifts to Z Promoted gapBlocked ->gapOpen Cat 2.",
    ...
    scope := "CLOSED: Cat 2 PUBLISHED ->Cat 1 LEAN-CLOSED: V_56 Chern classes generate H*(BE_7; Z; Lean-internal closure via typeclass-field projection (theorem `kono_mimura_1976_V56_generates_BE7_OPEN`)" }
...
def gap_H8_EVII_one_dim : StrictGapEntry :=
  { name := "H8_EVII_one_dim_OPEN"
    ...
    attackHistory := ["P39: Cat 2 single-step; H^8(Ě_VII; Z = ℚ·h^4",
    ...
    scope := "H^8(Ě_VII; Z is 1-dimensional, spanned by h^4 --Cat 1 LEAN-CLOSED via the Borel-Hirzebruch partition-count coefficient computation (P94)" }
...
def gap_borel_serre_1973_franke_1998_eisenstein_layer : StrictGapEntry :=
  { name := "borel_serre_1973_franke_1998_eisenstein_layer_OPEN"
    ...
    scope := "CLOSED: Cat 2 PUBLISHED ->Cat 1 LEAN-CLOSED: H^*_Eis(S_Γ; Z Borel-Serre layer decomposition by proper Zparabolics; Lean-internal closure via typeclass-field projection (theorem `borel_serre_1973_franke_1998_eisenstein_layer_OPEN`)" }
...
def gap_e7_min_parabolic_BS_codim : StrictGapEntry :=
  { name := "e7_min_parabolic_BS_codim_OPEN"
    ...
    attackHistory := ["P55: Cat 2 single-step; E_7 root-system fact --minimum proper Zparabolic Borel-Serre stratum codim is 26 (E_6-Levi maximal parabolic)",
    ...
    scope := "Every proper Zparabolic of E_{7(-25)} has Borel-Serre stratum codim ~26; minimum achieved by the E_6 × T_1 maximal parabolic (dim N_P = 27, split-center rank 1)" }
...
def gap_freudenthal_scalar_piece_computation : StrictGapEntry :=
  { name := "freudenthal_scalar_piece_computation_OPEN"
    ...
    attackHistory := ["P39 introduction (2026-05-15): the (ab)^2 ->81 h^4 computation",
                      ...
                      "P45 hostile audit (2026-05-15): P44 forgot the O(1)-twist in TZV). CORRECT: N = 27'_{-4} ?1_{-6}; charge-consistency forces m forAll{2,3}; the base-point slice q(1,0,B,b) = b^2 + 4N(B) gives m = 2. Leading jet q_2 = b^2 = (ab)^2|_{a=1} forAll(Sym^2 N^forAll?O(4))^L = 1_0, L-invariant, NONZERO. P39's (ab)^2 focus RE-VINDICATED",
...
def gap_HC_Main : StrictGapEntry :=
  { name := "HC_for_freudenthal_quartic_on_EVII_UNCONDITIONAL"
    ...
    attackHistory := [
      ...
      "P32 + P36 audit-reframe: Hyp_VZ_AqLambda DROPPED --under Hyp_BorelMAtLeast8 j^8 iso, H^8(S_Γ; Z_G is 1-dim from trivial-module Cartan image (h^4); non-trivial A_q(λ) at R(q)=8 (if exists) doesn't contribute to freudenthal class. Earlier P32 'R(q)=8 NEVER ACHIEVED' verdict based on dim_C(u∩k_C) enumeration which is NOT V-Z R(q) := dim_C(u∩p_C); P36 hostile audit caught the mis-identification but the STRUCTURAL conclusion (Hyp_VZ_AqLambda redundant under Hyp_BorelMAtLeast8) stands.",
      "P34: Hyp_HigherRank_GoodMetric_OPEN DROPPED --Mumford 1977 Thm 3.1 is type-uniform for ANY automorphic ρ (covers V_56 on EVII directly) + Harris 1985 §4 algebraic upgrade + BKK 2007 Thm 5.2 + K_forAllisotypic V_56 = L_{+3} ?E_{+1} ?E_{-1} ?L_{-3} = full closure synthesis",
      ...
      "P55 CLOSED Hyp_Eisenstein_Vanishing: the Eisenstein cohomology H^*_Eis(S_Γ; Z of an arithmetic Γ ?E_{7(-25)}(Z decomposes by proper Zparabolic (Franke 1998 §1.4 + Borel-Serre 1973 boundary stratification + Borel-Wallach Ch. VII spectral sequence + Schwermer 1994 + Saper 2005), each layer contributing at degrees ~codim Y_P. The minimum codim across proper Zparabolics of E_7 is 26 (E_6-Levi maximal parabolic: dim N_P = 27, split-center rank 1 ?codim Y_P = 26; all other proper Zparabolics have strictly larger N_P). At target degree d = 8 < 26 every layer contributes zero, giving H^8_Eis(S_Γ; Z = 0. (Q-rank 0 case is trivial: cocompact, no boundary, no Eisenstein.) Encoded via borel_serre_1973_franke_1998_eisenstein_layer_OPEN + e7_min_parabolic_BS_codim_OPEN + eisenstein_vanishing_at_deg8_via_franke_layer_OPEN + Hyp_Eisenstein_Vanishing_DERIVED. Main Theorem 2 ->1 Hyp_*.",
      "P56 BYPASSED Hyp_BorelMAtLeast8: Hyp_BorelMAtLeast8 (= m(E_{7(-25)}) ~8 = full j^8 ISO) is OVER-STRONG. Proof chain only needs the INJECTIVE half --c(E_7) = 8 PUBLISHED via Borel 1974 §9.1(3) p.261 directly. With injectivity alone, the freudenthal class [q] := j^8(h^4) is a non-zero G-invariant (4,4)-Hodge class (G-equivariance of j^q from Borel 1974 §3-§8; Cartan thm for h^4 G-invariance on Ě_VII); algebraicity follows from j^8(h^4) = c_1(L̄)^4 via Borel-Hirzebruch 1958 + Mumford 1977 §1.3 canonical extension. The '1-dim H^8(S_Γ; Z_G' reading (surjectivity-dependent) was paper narrative, NOT load-bearing. Encoded via borel_1974_c_E7_eq_8_PUBLISHED_OPEN (no Hyp_* input) + refactored paper_placement_reduction_OPEN (takes cohomologyIso_at_deg8 instead of Hyp_BorelMAtLeast8) + cascade-unconditional DERIVED theorems. Main Theorem 1 ->0 Hyp_* (UNCONDITIONAL).",
...
-- (g,K)-cohomology iso H^*(g,K;Z = H^*(Ě;Z) making the trivial-module
