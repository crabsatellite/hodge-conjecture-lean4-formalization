...
def config : ChainAudit.ProjectConfig := {
  ...
  researchChains := [
    ...
    },
    {
      ...
      title := "HCGapL4 multi-front attack waves (R420 -- R775)"
      kind := "active"
      status := "exploratory"
      summary :=
      ...
      successCriterion :=
        "A successful follow-up closes one live H8 residual target without adding assumptions.  The current R775 preferred attack is explicit: prove exact image `Submodule.map j_q source_invariants = surjectivity_target`, prove the concrete generator-membership theorem `h^4 in compactDual`, and prove exact target-line equality `target_invariants = span {j_q(h^4)}`.  R775 proves that, once compact-dual generator membership is present, exact target-line equality is equivalent to the scalar-preimage certificate and feeds the same target-line equality, boundary data, and target boundary consumers.  R774 proves the generator-membership field is equivalent to the R772 Cartan-containment field for the carrier consumers; R773 proves exact image is still independent of the Cartan/scalar side, even after the R772 carrier closure, so exact image must be proved from genuine Matsushima source geometry or from a separate theorem implying it.  Older equivalent surfaces remain valid attack spellings: scalar-preimage certificate, Cartan-to-compactDual containment, compact-dual/H8, source-invariants/H8, quotient vanishing, finite trivial-module multiplicity, reverse Cartan-image containment, and exact target-line equality.  The guardrails still apply: exact image remains independent of the other abstract targets, boundary data alone does not prove the non-boundary target, paper-facing carrier stacks alone do not prove the needed compact-dual generator/Cartan or target-line fields, and finite/rank/cardinality wrappers must not replace the explicit target-line equality or its equivalent scalar certificate."
    },
    {
