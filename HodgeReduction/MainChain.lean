  ...
  researchChains := [
    ...
    },
    {
      ...
      attackPlan := [
        ...
        "R726 status: that Cartan comparison is exactly the two containment targets `compactDual <= CartanH8` and `CartanH8 <= compactDual`; paper-facing carriers plus boundary data force neither direction, so the next attack should choose one containment and prove it from real EVII compact-dual geometry.",
