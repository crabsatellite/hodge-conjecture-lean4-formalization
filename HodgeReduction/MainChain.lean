...
def config : ChainAudit.ProjectConfig := {
  ...
  researchChains := [
    ...
    },
    {
      ...
      successCriterion :=
        "A successful follow-up closes one live H8 residual target without adding assumptions: prove `MatsushimaV56BoundaryData`, prove `source_invariants = H8` (equivalently `compactDual = CartanH8`), prove the target-line theorem under boundary data, or prove the full R685 two-target route."
    },
    {
