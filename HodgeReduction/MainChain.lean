  ...
  researchChains := [
    ...
    },
    {
      ...
      attackPlan := [
        ...
        "R768 next attack: prove exact image `Submodule.map j_q source_invariants = surjectivity_target`, prove source-H8, construct the scalar-preimage certificate, and prove nonzero `trivialModulePart` or nonzero finrank from EVII automorphic geometry.",
      ...
      successCriterion :=
        "A successful follow-up closes one live H8 residual target without adding assumptions.  The current R768 preferred attack is explicit: prove exact image `Submodule.map j_q source_invariants = surjectivity_target`, prove `source_invariants = H8`, construct the scalar-preimage certificate `forall beta in target_invariants, exists r, r smul j_q(h^4) = beta`, and prove a nonzero `trivialModulePart` witness or nonzero finite-dimensional `trivialModulePart` finrank.  R768 rebuilds `MatsushimaV56BoundaryData` from those fields; R766/R767 feed target-line equality, target-generator membership, and `compactDual = H8`; R765/R764 then feed the source-H8 consumers.  Older equivalent surfaces remain valid attack spellings: compact-dual/H8, source-invariants/H8, quotient vanishing, finite trivial-module multiplicity, reverse Cartan-image containment, and exact target-line equality.  The guardrails still apply: boundary data alone does not prove the non-boundary target, paper-facing carrier stacks alone do not prove the needed compact-dual/Cartan or target-line fields, and finite/rank/cardinality wrappers must not replace the explicit scalar certificate or nonzero witness."
    },
    {
