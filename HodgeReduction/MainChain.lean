...
def config : ChainAudit.ProjectConfig := {
  ...
  researchChains := [
    ...
    },
    {
      ...
      title := "HCGapL4 multi-front attack waves (R420 -- R771)"
      kind := "active"
      status := "exploratory"
      summary :=
      ...
      attackPlan := [
        ...
        "R771 next attack: prove exact image `Submodule.map j_q source_invariants = surjectivity_target`, prove `CartanH8 <= compactDual`, and construct the scalar-preimage certificate from EVII Matsushima/Borel--Wallach multiplicity or quotient-vanishing geometry.",
      ...
      successCriterion :=
        "A successful follow-up closes one live H8 residual target without adding assumptions.  The current R771 preferred attack is explicit: prove exact image `Submodule.map j_q source_invariants = surjectivity_target`, prove `CartanH8 <= compactDual`, and construct the scalar-preimage certificate `forall beta in target_invariants, exists r, r smul j_q(h^4) = beta`.  R771 proves the Cartan containment supplies source generator membership; R770 then recovers source-H8 from the scalar certificate, R769 supplies the concrete target-nonzero certificate, R768 rebuilds `MatsushimaV56BoundaryData`, and R766/R767 feed target-line equality, target-generator membership, and `compactDual = H8`.  Older equivalent surfaces remain valid attack spellings: compact-dual/H8, source-invariants/H8, quotient vanishing, finite trivial-module multiplicity, reverse Cartan-image containment, and exact target-line equality.  The guardrails still apply: exact image remains independent of the other abstract targets, boundary data alone does not prove the non-boundary target, paper-facing carrier stacks alone do not prove the needed compact-dual/Cartan or target-line fields, and finite/rank/cardinality wrappers must not replace the explicit scalar certificate."
    },
    {
