...
def config : ChainAudit.ProjectConfig := {
  ...
  researchChains := [
    ...
    },
    {
      ...
      title := "HCGapL4 multi-front attack waves (R420 -- R673)"
      kind := "active"
      status := "exploratory"
      summary :=
      ...
      successCriterion :=
        "A successful follow-up closes one live H8 residual target without adding assumptions: prove exact image, prove `MatsushimaV56BoundaryData` (R672 consumes it for exact image and, with source-H8, the target side), prove `CartanH8 <= compactDual` (equivalently `h^4 ∈ compactDual`), or prove the target-side theorem in any equivalent spelling: `targetInvariantExcessQuotient = bot`, `target_invariants <= span {j_q(h^4)}`, `target_invariants = span {j_q(h^4)}`, or bundled finite-dimensional target invariants plus `finrank target_invariants <= shimuraEVIIExpectedBetti 8`."
    },
    {
