...
def config : ChainAudit.ProjectConfig := {
  ...
  researchGaps := [
    ...
    },
    {
      ...
      title := "HCGapL4 multi-front Layer-4 attack waves (R420 -- R660)"
      status := "active-open"
      summary :=
  ...
  researchChains := [
    ...
    },
    {
      ...
      title := "HCGapL4 multi-front attack waves (R420 -- R660)"
      kind := "active"
      status := "exploratory"
      summary :=
      ...
      successCriterion :=
        "A successful follow-up closes one live H8 residual target without adding assumptions: prove exact image, prove `h^4 ∈ compactDual`, or prove `targetInvariantExcessQuotient = bot` / `trivialModulePart <= span {j_q(h^4)}`."
    },
    {
