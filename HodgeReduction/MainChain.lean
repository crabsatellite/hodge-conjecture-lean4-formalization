...
def config : ChainAudit.ProjectConfig := {
  ...
  researchChains := [
    ...
    },
    {
      ...
      title := "HCGapL4 multi-front attack waves (R420 -- R772)"
      kind := "active"
      status := "exploratory"
      summary :=
      ...
      attackPlan := [
        ...
        "R772 next attack: prove exact image `Submodule.map j_q source_invariants = surjectivity_target`, prove `CartanH8 <= compactDual`, and construct the scalar-preimage certificate from EVII Matsushima/Borel--Wallach multiplicity or quotient-vanishing geometry.",
