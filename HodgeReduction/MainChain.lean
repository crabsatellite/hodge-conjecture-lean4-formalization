...
def config : ChainAudit.ProjectConfig := {
  ...
  researchGaps := [
    ...
    },
    {
      ...
      title := "HCGapL4 multi-front Layer-4 attack waves (R420 -- R684)"
      status := "active-open"
      summary :=
  ...
  researchChains := [
    ...
    },
    {
      ...
      successCriterion :=
        "A successful follow-up closes one live H8 residual target without adding assumptions: prove `MatsushimaV56BoundaryData`, prove `compactDual = CartanH8` (equivalently `compactDual = H8`), prove the target-side theorem in any equivalent spelling (`targetInvariantExcessQuotient = bot`, `target_invariants <= span {j_q(h^4)}`, `target_invariants = span {j_q(h^4)}`, or bundled finite-dimensional target invariants plus `finrank target_invariants <= shimuraEVIIExpectedBetti 8`), or prove the R684 two-target route."
    },
    {
