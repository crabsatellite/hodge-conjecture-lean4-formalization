...
def config : ChainAudit.ProjectConfig := {
  ...
  researchGaps := [
    ...
    },
    {
      ...
      title := "HCGapL4 multi-front Layer-4 attack waves (R420 -- R756)"
      status := "active-open"
      summary :=
        "Active exploratory attack waves on the L4 / cohomology-profile / connectedness pipeline: FrontA (Deligne H0 sheaf realization), FrontB (Baily--Borel connectedness), FrontC (E_7 low-degree Hodge numbers + Hodge polynomial algebra + all-degree rank adapter + EVII/V56/Shimura expected Betti profile), FrontD (E_7 -> CM Chow correspondence + Deligne 1982 minimal fragment), FrontE (real-carrier profile matching + R405 conditional transfer feed).  Audits R451 / R456 / R460 / R465 / R470 / R476 are wave-level summaries.  R552 certifies the expected Shimura Betti profile degree-by-degree from EVII compact-dual Hodge sums plus the isolated V56 degree-3 contribution; R553 ties that finite V56 contribution to the actual `PureHodgeStructure V56 3` infrastructure; R554 combines the Matsushima, Eisenstein, and cuspidal trivial-module infrastructure into an honest boundary theorem; R555 proves the Cartan compact-dual source bridge and reduces the R554 source equality to `surjectivity_source = source_invariants`; R556 turns both source/target boundary equalities into finite-dimensional containment plus finrank obligations, routing the target through the cuspidal trivial-module part; R557 shows target containment is forced by source containment, and the later R560--R706 files progressively normalize and obstruct the compact-dual/source-invariant residual without adding hidden premises.  R707 proves the no-extra compact-dual carrier fact is equivalent to finite-dimensional `compactDual`, `finrank compactDual <= 1`, and the same `h^4 in compactDual` generator target.  R708 sharpens this: under finite-dimensionality and `h^4 in compactDual`, both `compactDual <= H8` and `compactDual = H8` are equivalent to the rank bound `finrank compactDual <= 1`; countermodels show finite-dimensionality plus generator membership cannot replace the rank bound, and the rank bound cannot replace generator membership.  R709 transfers this finite-rank carrier contract exactly to the primitive Matsushima `source_invariants` spelling via `compactDual = source_invariants`, leaving boundary data, source generator membership, source finite-dimensionality, and source rank <= 1 as the live source-side targets.  R710 sharpens that source-side route: with source finite-dimensionality and `h^4 in source_invariants`, both `source_invariants <= H8` and `source_invariants = H8` are equivalent to `finrank source_invariants <= 1`; countermodels block dropping either the source rank bound or source generator membership.  R711 connects this finite-rank source contract to the R705 carrier split, R712 blocks dropping explicit finite-dimensionality, R713 rewrites source generator membership as `H8 <= source_invariants`, R714 proves that this H8-containment finite-rank contract feeds the boundary/source-H8 and R705 compact-dual carrier-split consumers directly, R715 shows that `source_invariants <= H8` supplies both finite-dimensionality and the rank bound, R716 packages the two source containments as the single source-H8 equality target `source_invariants = H8`, R717 identifies that source-H8 equality target with the compact-dual carrier theorem `compactDual = H8`, and R718 reconnects the current route to the sharp compact-dual finite-rank attack surface: boundary data, `h^4 in compactDual`, finite-dimensional `compactDual`, and `finrank compactDual <= 1`.  R719 unfolds the boundary-data input into the primitive equalities `surjectivity_source = compactDual` and `surjectivity_target = trivialModulePart`, so the live finite-rank route has five explicit primitive targets.  R720 rewrites the generator target `h^4 in compactDual` as the geometric containment `H8 <= compactDual`, and finite rank plus that containment directly recovers `compactDual = H8`.  R721 collapses the finite-dimensionality and rank-bound fields to the opposite containment `compactDual <= H8`, so the live route now has four primitive targets: the two boundary equalities plus the two compact-dual containment directions.  R722 proves this four-target primitive route is exactly the two-target geometry route `MatsushimaV56BoundaryData` plus `compactDual = H8`; neither target is asserted.  R723 records that the two R722 targets are independent in the current abstract interface: boundary data alone does not force `compactDual = H8`, and `compactDual = H8` alone does not force boundary data or the R722 contract.  R724 adds the paper-facing GK/Borel-Wallach/BBW/Freudenthal carrier stack to the boundary countermodel and still refutes `compactDual = H8`, so the next proof must supply a genuine EVII Matsushima compact-dual comparison theorem.  R725 rewrites the latest R722 primitive route as the equivalent Cartan/GK comparison target `compactDual = CartanCompactDualIso.trivialModuleGK_H8` plus boundary data, and transports the R724 countermodel to show the paper-facing carrier stack still does not force the Cartan comparison.  R726 splits that comparison into `compactDual <= CartanH8` and `CartanH8 <= compactDual`, and equips the second R706/R664 countermodel with the same carrier stack to show neither direction is forced abstractly.  R727 chooses the generator direction, proves `CartanH8 <= compactDual` is equivalent to the concrete geometry theorem `h^4 in MatsushimaCompactDualData.compactDual`, and rewrites the R726 route as boundary data plus `compactDual <= CartanH8` plus that generator witness.  R728 proves this current R727 route is equivalent to boundary data plus the single source-invariant carrier theorem `source_invariants = H8`.  R729 opens that equality into boundary data, source generator membership, and target generator-line containment; the generator-line contract is equivalent to the current route, and the target line containment controls the no-extra source half.  R730 transports the generator field across `compactDual = source_invariants`, so the current equivalent route is boundary data, `h^4 in compactDual`, and the same target generator-line containment.  R731 replaces that line route by the direct compact-dual two-containment route: boundary data, `compactDual <= H8`, and `H8 <= compactDual`.  R732 transports the R726 carrier-stack countermodels to this direct spelling: GK/Borel-Wallach/BBW/Freudenthal carriers plus boundary data still force neither `compactDual <= H8` nor `H8 <= compactDual`.  R733 proves that, after boundary data and `H8 <= compactDual`, the no-extra target `compactDual <= H8` is equivalent to `trivialModulePart <= span {j_q(h^4)}`.  R734 proves that, under the same `H8 <= compactDual`, that target-line theorem is equivalent to finite-dimensional `trivialModulePart` plus `finrank trivialModulePart <= 1`, with the finite-dimensional witness explicit.  R735 records the matching guardrail: even adding paper-facing carriers and boundary data to `H8 <= compactDual` does not force that finite multiplicity, so the target remains genuine automorphic geometry.  R736 reconnects that finite-multiplicity target to the older quotient/scalar-preimage surface: the current route is equivalently boundary data, `H8 <= compactDual`, `source_invariants = H8`, and `targetInvariantExcessQuotient = bot`, so finite multiplicity, line containment, scalar preimages, and quotient vanishing are one target-side gap.  R737 proves the R736 H8-containment field is redundant once source-H8 is part of the quotient route: boundary data plus `source_invariants = H8` plus `targetInvariantExcessQuotient = bot` is equivalent to the R736 four-field route, and boundary data supplies the older R641 exact-image field.  R738 proves that the R737 quotient field itself is forced by boundary data plus source-H8 via the R729 line theorem and R658 quotient/line equivalence, so the current preferred L4 residual is again a two-target route.  R739 rewrites that route as boundary data plus the target-invariant line equality `target_invariants = span {j_q(h^4)}`, deriving source-H8 and quotient vanishing as consumers.  R740 splits that target-line equality into two primitive half-targets: `j_q(h^4) in target_invariants` and `target_invariants <= span {j_q(h^4)}`.  No new axioms.  R741 transports the generator-membership half across boundary data: under `MatsushimaV56BoundaryData`, `j_q(h^4) in target_invariants` is equivalent to `H8 <= compactDual`, so the current preferred route is boundary data plus `H8 <= compactDual` plus `target_invariants <= span {j_q(h^4)}`.  R742 normalizes that no-extra target-invariant line to explicit finite rank-one `trivialModulePart` multiplicity under the same H8-containment target, leaving boundary data, H8 containment, finite-dimensional `trivialModulePart`, and `finrank trivialModulePart <= 1` as the preferred current route.  R743 rewrites that finite rank-one target, under H8 containment, as the element-level Cartan scalar-preimage theorem, leaving boundary data, H8 containment, and scalar preimage as the preferred current route.  R744 rewrites that scalar target, under boundary data and H8 containment, as exact Cartan image `Submodule.map j_q CartanH8 = trivialModulePart`, so the preferred current route is boundary data, H8 containment, and exact Cartan image.  R745 observes that H8 containment supplies the easy inclusion `Submodule.map j_q CartanH8 <= trivialModulePart`, so the remaining exact-image target is the reverse containment `trivialModulePart <= Submodule.map j_q CartanH8`.  R746 proves this latest three-field route is equivalent to the older two-target route `MatsushimaV56BoundaryData` plus `compactDual = CartanH8`, so the live frontier is boundary data plus the Cartan/compact-dual carrier equality.  R747 reconnects that frontier to the sharp finite-rank compact-dual attack surface: prove boundary data, prove `h^4 in compactDual`, prove finite-dimensional `compactDual`, and prove `finrank compactDual <= 1`; this is equivalent to the Cartan carrier equality and is not a closure claim.  R748 merges finite-dimensionality plus `finrank <= 1`, under the same nonzero generator membership, into the exact target `finrank compactDual = 1`, so the live frontier is boundary data, generator placement, and rank one.  R749 proves generator placement plus exact compact-dual rank one is equivalent to the single carrier theorem `compactDual = H8`, so the live frontier is again boundary data plus compact-dual/H8.  R750 transports the R724 carrier-stack countermodel to this rank-one generator spelling: paper-facing carriers plus boundary data still do not force `h^4 in compactDual` together with `finrank compactDual = 1`.  R751 rewrites the current two-target frontier as three primitive targets: `compactDual = H8`, `surjectivity_source = compactDual`, and `finrank trivialModulePart = 1`; this reduces the target boundary field to automorphic multiplicity without closing any target.  R752 splits that exact rank-one target into explicit finite-dimensionality, `finrank trivialModulePart <= 1`, and the nonvanishing lower-bound target `finrank trivialModulePart != 0`.  R753 proves that this nonvanishing target follows from the existing `compactDual = H8` target plus the finite-dimensional witness, reducing the live equivalent frontier to four targets: `compactDual = H8`, `surjectivity_source = compactDual`, finite-dimensional `trivialModulePart`, and `finrank trivialModulePart <= 1`.  R754 rewrites the finite-dimensional plus upper-bound pair, under `compactDual = H8`, as the single target-line containment `trivialModulePart <= span {j_q(h^4)}`, so the current equivalent frontier has three targets."
      files := [
  ...
  researchChains := [
    ...
    },
    {
      ...
      attackPlan := [
        ...
        "R756 next attack: prove one of the three remaining fields directly, with priority on the genuine EVII comparison `compactDual = H8` or the source equality `surjectivity_source = compactDual`; the target-side Cartan containment is now fully wired to boundary-data consumption.",
      ...
      successCriterion :=
        "A successful follow-up closes one live H8 residual target without adding assumptions: prove `MatsushimaV56BoundaryData`, prove both `compactDual <= H8` and `h^4 in compactDual` from genuine EVII compact-dual/source-invariant geometry, prove `compactDual = H8`, prove `source_invariants = H8` or the target-line theorem under boundary data, prove the R687 generator/multiplicity route by closing boundary data, h^4 source membership, and finite-dimensional trivial-module upper bound, prove the R688/R689/R690 smaller route by closing source boundary equality, h^4 source membership, finite-dimensional trivialModulePart, and `finrank <= 1`, prove the R691 route by closing source boundary equality, h^4 source membership, and `trivialModulePart <= span {j_q(h^4)}`, prove the R692/R693/R694/R696/R703/R704/R705 route by closing `MatsushimaV56BoundaryData` and either both compact-dual carrier split targets or one equivalent spelling (`compactDual = H8`, `surjectivity_source = H8`, `source_invariants = H8`, or the target-line equality), prove the R697/R698/R700/R701/R702 route by closing the equivalent R705 route, prove the R709/R710/R711/R712/R713/R714/R715/R716/R717/R718/R719/R720/R721 primitive compact-dual two-containment route by closing `surjectivity_source = compactDual`, `surjectivity_target = trivialModulePart`, `compactDual <= H8`, and `H8 <= compactDual` (equivalently the R720 finite-rank route and the R719 generator target) (equivalently boundary data plus `compactDual = H8`, equivalently `source_invariants = H8`, equivalently the two R715 source-H8 containments), or prove the R731 current compact-dual two-containment route by closing boundary data, `compactDual <= H8`, and `H8 <= compactDual`.  R686 blocks treating boundary data alone as a source-H8 proof; R693 blocks consuming h^4 source membership from source-H8 plus line containment; R694 shows boundary data plus source-H8 is sufficient for the R692 generator-line fields; R695 shows boundary data alone cannot provide source-H8; R696 shows source-H8/source-invariants-H8/target-line are one non-boundary target under boundary data; R704 shows source-H8 is just compactDual-H8 once boundary data is fixed; R705 shows compactDual-H8 is exactly `compactDual <= H8` plus `h^4 in compactDual`; R706 blocks deriving either R705 carrier fact from boundary data alone; R707 shows the no-extra compactDual carrier target can equivalently be attacked as finite-dimensional `compactDual` plus `finrank compactDual <= 1` together with the same `h^4 in compactDual` generator target; R708 shows that once finite-dimensionality and `h^4 in compactDual` are fixed, the remaining no-extra/equality target is exactly the rank bound, and it blocks dropping either generator membership or rank from the route; R709 transfers the same contract to source_invariants without strengthening it; R710 makes the source-invariant version sharp under the same finite/generator preconditions; R711 connects that source finite-rank route back to the R705 carrier-split route so future work should close the source-side obligations rather than treating the contract surfaces as separate gaps; R712 blocks dropping source finite-dimensionality merely because a naked finrank inequality typechecks; R713 rewrites source generator membership as the H8-containment theorem that geometry should now attack; R714 proves that the R713 contract is an immediate consumer for the boundary/source-H8 and R705 carrier-split routes; R715 shows source no-extra containment supplies finite-dimensionality and rank, R716 packages the source side as the single equality `source_invariants = H8`, R717 identifies that equality with `compactDual = H8`, and R718 identifies the latest route with the sharp compact-dual finite-rank attack surface, R719 unfolds its boundary-data input into two primitive boundary equalities, R720 rewrites its generator item as `H8 <= compactDual`, R721 replaces the finite/rank fields by `compactDual <= H8`, R722 collapses the primitive route back to the two geometric targets boundary data plus `compactDual = H8`, and R723 blocks deriving either target from the other inside the current abstract interface; R724 blocks deriving `compactDual = H8` or the R722 contract from the paper-facing GK/Borel-Wallach/BBW/Freudenthal carrier stack even when boundary data is present, so a genuine EVII Matsushima compact-dual comparison theorem remains required; R725 identifies that same target with `compactDual = CartanCompactDualIso.trivialModuleGK_H8` and blocks deriving the Cartan/GK comparison from the carrier stack, so the next proof must attack the compact-dual-to-Cartan comparison directly; R726 splits that comparison into `compactDual <= CartanH8` and `CartanH8 <= compactDual` and blocks deriving either direction from paper-facing carriers plus boundary data; R727 isolates the second direction as the exact generator-placement theorem `h^4 in compactDual`; R728 compresses the current route back to boundary data plus `source_invariants = H8`; R729 proves the equivalent generator-line route and shows the target line containment controls the no-extra source half; R730 shows the generator field is equivalently `h^4 in compactDual`, R731 shows the current route is equivalently boundary data plus `compactDual <= H8` and `H8 <= compactDual`, R732 blocks deriving either direction from carrier stack plus boundary data alone, and R733 identifies the no-extra direction with target-line containment once boundary data and `H8 <= compactDual` are fixed; R734 identifies that target-line theorem with explicit finite trivial-module multiplicity under the same H8 containment; R735 blocks deriving that multiplicity from carrier stack plus boundary and H8 containment alone; R736 identifies that multiplicity target with the quotient/scalar-preimage surface; R737 removes `H8 <= compactDual` as an independent target when `source_invariants = H8` is part of the quotient route, R738 proves quotient vanishing is derived from boundary data plus source-H8, R739 rewrites source-H8 under boundary data as the target-invariant line equality, and R740 splits that equality into generator membership plus no-extra containment; R741 transports the generator-membership half across boundary data to `H8 <= compactDual`; R742 rewrites the no-extra target-invariant line under H8 containment as finite-dimensional `trivialModulePart` plus `finrank trivialModulePart <= 1`; R743 rewrites that finite rank-one target under H8 containment as the element-level Cartan scalar-preimage theorem; R744 rewrites that scalar target under boundary data and H8 containment as exact Cartan image; R745 reduces exact Cartan image under H8 containment to the reverse inclusion because the forward inclusion is automatic; R746 proves that boundary data plus H8 containment plus that reverse inclusion is equivalent to boundary data plus `compactDual = CartanH8`; R747 proves that this same frontier is equivalent to boundary data, `h^4 in compactDual`, finite-dimensional `compactDual`, and `finrank compactDual <= 1`; R748 proves that, with `h^4 in compactDual`, the finite-dimensionality plus upper-bound pair is equivalent to exact `finrank compactDual = 1`; R749 proves the generator plus exact-rank-one fields are equivalent to the compact-dual/H8 carrier theorem; R750 blocks deriving that generator-plus-rank-one pair from the current paper-facing carrier stack plus boundary data. Therefore the next preferred attack is to prove `MatsushimaV56BoundaryData` and prove `compactDual = H8` (equivalently prove `h^4 in compactDual` plus `finrank compactDual = 1`) from genuine EVII compact-dual geometry, not from the existing carrier stack alone. R697/R698/R700/R701/R702/R703/R704/R705 identify that non-boundary target with the finite trivial-module multiplicity computation and connect it back to the R690 explicit route, the R646 Cartan-image containment route, and the preferred boundary-data/compact-dual-carrier route."
    },
    {
