...
def config : ChainAudit.ProjectConfig := {
  ...
  openAxioms := [
    ...
    -- Paper-citation axiom for thm_cy3_e7_nonexistence (paper Â§4
  ...
  researchGaps := [
    ...
    },
    {
      ...
      status := "open"
      summary :=
        "AMRT 1975 / Baily--Borel 1966 construction of S_Î“^tor as a SmoothProjectiveVariety â„?  Required Mathlib infrastructure: arithmetic groups, Hermitian symmetric domains, toroidal compactifications."
      files := [
        "HodgeReduction/OpenHypotheses.lean",
        "HodgeReduction/HCGapRegistry.lean",
    ...
    },
    {
      ...
      status := "open"
      summary :=
        "Construction of `VarietyCohomologyData` whose `H k` is the actual rational singular cohomology of `S_Î“^tor` at degree `k`, with `hodgeStructure k` the actual pure Hodge structure of weight `k` on `H^k(S_Î“^tor, â„?`.  Required Mathlib infrastructure: singular cohomology, Dolbeault decomposition, Hodge theorem for compact KÃ¤hler manifolds."
      files := [
        "HodgeReduction/HCGapRegistry.lean",
    ...
    },
    {
      ...
      status := "open"
      summary :=
        "Per-codimension data: HodgeStructureMorphism phi + cycle map psi + commuting square + Hodge-class surjectivity, identifying H^*(S_Î“^tor, â„? with a CM abelian variety's cohomology.  Currently bundled inside `canonicalE7ShimuraTor.mtCorrespondencePackage` (R188 field); a `mt_correspondence_e7_witness_exists` universal axiom (R177) covers the non-canonical sub-clause."
      files := [
        "HodgeReduction/MainTheorem.lean",
        "HodgeReduction/OpenHypotheses.lean",
        "HodgeReduction/HCGapRegistry.lean",
  ...
  researchChains := [
    ...
    },
    {
      ...
      summary :=
        "Meyer / G_2 / F_4 / E_8 vacuity are kernel-pure derived theorems.  `thm_cy3_e7_nonexistence` still consumes `cy3_e7_nonexistence_paper_axiom` (paper Â§4 Stages A--D)."
      files := [
        "HodgeReduction/ClassicalResults.lean",
        "HodgeReduction/MainTheorem.lean"
      ]
    ...
    },
    {
      ...
      status := "closed-toy"
      summary :=
        "`HC_for_Concrete_EVII` specialises the abstract closure `HC_for_freudenthal_quartic_on_EVII_UNCONDITIONAL` to a concrete `A_EVII := Polynomial â„š` toy carrier.  Cone is `{propext, Classical.choice, Quot.sound}` (no project axioms) but the carrier is explicitly toy; per R201 mandate it is EXCLUDED from real-HC closure accounting."
      files := [
