...
def config : ChainAudit.ProjectConfig := {
  ...
  researchChains := [
    ...
    },
    {
      ...
      title := "HCGapL4 multi-front attack waves (R420 -- R674)"
      kind := "active"
      status := "exploratory"
      summary :=
      ...
      successCriterion :=
        "A successful follow-up closes one live H8 residual target without adding assumptions: prove `MatsushimaV56BoundaryData`, prove the target-side theorem in any equivalent spelling (`targetInvariantExcessQuotient = bot`, `target_invariants <= span {j_q(h^4)}`, `target_invariants = span {j_q(h^4)}`, or bundled finite-dimensional target invariants plus `finrank target_invariants <= shimuraEVIIExpectedBetti 8`), or prove exact image / source-H8 directly outside the boundary-data route."
    },
    {
