...
def config : ChainAudit.ProjectConfig := {
  ...
  researchChains := [
    ...
    },
    {
      ...
      title := "HCGapL4 multi-front attack waves (R420 -- R757)"
      kind := "active"
      status := "exploratory"
      summary :=
      ...
      attackPlan := [
        ...
        "R757 next attack: prove one of the three remaining fields directly, with priority on a genuine EVII comparison for `compactDual = H8` or a new Matsushima source theorem for `surjectivity_source = compactDual`; do not route through only the existing carrier stack.",
